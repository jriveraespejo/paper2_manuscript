...
dispersion}, reflects variability in the perceived trait level of the
stimulus. Figure~\ref{fig-discriminal_process} shows example
...
are not directly observable without comparing it to other stimuli, the
\emph{law of comparative judgment} becomes essential. This law asserts
that when assessing a specific trait by comparing two stimuli, the
stimulus positioned further along the continuum is perceived as having a
higher level of that trait. Thus, the observed dichotomous outcome is
determined by the distribution of the difference between the discriminal
processes of the two stimuli, called the \emph{discriminal difference}.
...
progressively simplifying assumptions. Figure~\ref{fig-thurstone_cases}
...
dispersion across stimuli, the correlation between stimuli, and which
...
\includegraphics[width=0.85\linewidth,height=\textheight,keepaspectratio]{./images/tables/thurstone_cases.png}

}

\caption{\label{fig-thurstone_cases}Thurstones cases and asumptions}
...
assumptions \citep[pp.~253]{Bramley_2008}, Case V remains the most
widely used case in the CJ literature. This popularity is largely due to
its simplified statistical representation in the Bradley-Terry-Luce
(BTL) model \citep{Bradley_et_al_1952, Luce_1959}. The BTL model mirrors
Case V's assumptions, with one key difference: while Case V assumes a
Normal distribution for the discriminal processes, the BTL model uses
the more mathematically tractable Logistic distribution
\citep[pp.~254]{Bramley_2008} (see Figure~\ref{fig-thurstone_cases}).
This substitution has little effect on the model's estimation or
interpretation, as the Normal and Logistic distributions differ by a
...
over precision in trait measurement. As a result, its assumptions may
not be suitable for applications beyond the psychophysical contexts,
which typically involve simple, homogeneous stimuli. Thurstone himself
cautioned that its use ``should not be made without (an) experimental
test'' \citep[pp.~270]{Thurstone_1927b}, further acknowledging that some
assumptions could become problematic with less homogeneous stimuli, such
as handwriting or English compositions \citep[pp.~374]{Thurstone_1927a}.
Consequently, given that current CJ applications often involve more
complex, less homogeneous stimuli, two assumptions of Case V may not
always hold theoretically or in practice: the assumption of zero
correlation and equal dispersion between stimuli.

{Regarding the assumption of \emph{zero correlation between stimuli}
\ldots{}}
...
typically functions as the measurement model for the trait under
assessment \citep{Andrich_1978, Bramley_2008}. A measurement model
...
of the texts (the latent variable)
\citep{Laming_2004, Pollitt_2012b, Whitehouse_2012, vanDaal_et_al_2016, Lesterhuis_2018, Coertjens_et_al_2017, Goossens_et_al_2018, Bouwer_et_al_2023}.
