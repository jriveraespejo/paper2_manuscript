...
statistical handling of this extra source of dependency between the
stimuli has been inadequate. For example, when CJ data includes multiple
samples of stimuli from the same individuals, researchers often rely on
...
compromise the validity of results derived from the data modeling.

Fortunately, the same literature offers solutions for addressing the
factors that create these dependencies between stimuli.
\citet{Andrich_1978} and \citet{Wainer_et_al_1978} recommend integrating
judges' biases into the BTL model. Furthermore, the literature advocates
for incorporating relevant hierarchical structures into the data model
to account for these dependencies. Taken together, these additions can
result in a model that combines a Multilevel Structural Equation Model
(MSEM) \citep[chap.~26]{Hoyle_et_al_2023} with a multidimensional or
two-parameter logistic IRT model \citep[chap.~15]{Hoyle_et_al_2023},
depending on the theoretical assumptions a researcher makes about the
judges' biases.
