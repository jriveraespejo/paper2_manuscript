...
inference uses directed acyclic graphs (DAG) and its associated
...
indicates that the edges of the graph extend from one node to another,
with arrows showing the direction of causal influence. Moreover, the
term \emph{acyclic} indicates the causal influences do not form a loop,
meaning the influences do not cycle back on themselves
\citep{McElreath_2020}.

One key advantage of DAGs is that they can represent various causal
structures using only five fundamental building blocks, regardless of
complexity. The left panels of Figure~\ref{fig-dags_scms} illustrates
the \emph{simplified} graphs of these block. In these graphs, the
variables of interest or \emph{endogenous} variables, \(V=\{X,Z,Y\}\),
are depicted as solid black circles, indicating that they are observed
variables. The arrows in the graph reflect the expected direction of
causal influences among the variables.

In contrast, the middle panels of Figure~\ref{fig-dags_scms} depicts the
\emph{magnified} graphs of these building blocks. These graph include,
in addition to the endogenous variables, the \emph{exogenous} variables
\(E=\{e_{X},e_{Z},e_{Y}\}\). These exogenous variables, commonly
referred to as \emph{disturbances} or \emph{errors}, represent factors
not explicitly modeled. Moreover, they are depicted as open circles,
indicating their unobserved nature. These exogenous variables are often
omitted for simplicity, resulting in the simplified DAGs shown in the
left panels. However, including them in the graph can be advantageous in
certain scenarios, as they can help to highlight potential issues
...
\includegraphics[width=0.6\textwidth,height=\textheight]{images/figures/sdag_bb3.png}
...
\includegraphics[width=0.6\textwidth,height=\textheight]{images/figures/sdag_bb4.png}
...
\includegraphics[width=0.6\textwidth,height=\textheight]{images/figures/sdag_bb5.png}
...
DAGs with corresponding SCMs. \textbf{Note:} left column shows
simplified DAGS, middle column show magnified DAGs, and right column
shows the corresponding SCMs.}

\end{figure}%

A careful examination of the left and middle panels of
Figure~\ref{fig-dags_scms} reveal what do the building blocks assume.
Figures \ref{fig-sdag_bb1} and \ref{fig-mdag_bb1} depict two unconnected
nodes, representing an scenario where variables \(X\) and \(Y\) are not
causally related. Figures \ref{fig-sdag_bb2} and \ref{fig-mdag_bb2} show
two connected nodes, illustrating a scenario where a parent node \(X\)
exerts a causal influence on a child node \(Y\). Consequently, \(Y\) is
considered a \emph{descendant} of \(X\). Figures \ref{fig-sdag_bb3} and
\ref{fig-mdag_bb3} depict a \emph{chain} (or \emph{pipe}), where \(X\)
influences \(Z\), and \(Z\) influences \(Y\). In this configuration,
\(X\) is a parent node of \(Z\), and \(Z\) a parent node of \(Y\).
Furthermore, the DAG shows that \(X\) is an \emph{ancestor} of \(Y\) and
that the relationship between these variables is entirely
\emph{mediated} by \(Z\). Figures \ref{fig-sdag_bb4} and
\ref{fig-mdag_bb4} illustrate a \emph{fork}, where variables \(X\) and
\(Y\) are both influenced by \(Z\). In this scenario \(Z\) is a parent
node of both \(X\) and \(Y\). Finally, figures \ref{fig-sdag_bb5} and
...
\includegraphics[width=0.35\textwidth,height=\textheight]{images/figures/sdag_example1.png}

}

\subcaption{\label{fig-sdag_example1}Simplied DAG}
...
\includegraphics[width=0.65\textwidth,height=\textheight]{images/figures/mdag_example1.png}

}

\subcaption{\label{fig-mdag_example1}Magnified DAG}
...
\subcaption{\label{fig-scm_example1}Collider or inmorality}
