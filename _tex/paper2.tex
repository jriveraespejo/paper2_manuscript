...
\({n_{I} \cdot n_{A} \choose 2} \cdot n_{J} \cdot n_{K}\) rows and \(6\)
columns. The matrix's dimension results from the conceptual-population
model's assumption that researchers have access to data capturing all
repeated judgments made by every available judge for each pair of
stimuli produced by each pair of individuals in the population. Here,
the term \({n_{I} \cdot n_{A} \choose 2}\) represents the Binomial
coefficient, which quantifies the total number of individual-stimuli
comparison pairs within the conceptual population. The coefficient is
defined as follows:
...
Figure~\ref{fig-cj11} illustrates the sample-comparison model,
incorporating the sample mechanism. Following the convention of
\citet[pp.~499-516]{McElreath_2020}, the DAG~\ref{fig-cj11_dag} shows
...
\caption{\label{fig-cj11}Sample-comparison model including sampling
mechanism}
...
\caption{\label{fig-cj13}Sample-comparison model including sample and
