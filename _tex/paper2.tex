...
\citep{Kelly_et_al_2022}. Furthermore, although the CJ practice has
attempted to address the issue of hypothesis testing by using the
...
\citep{Pollitt_2004, Pollitt_2012b}. These misfit texts may exhibit
larger discriminal dispersions or represent outliers --texts with
distinctive characteristics that deviate markedly from the rest of the
sample \citep{Grubbs_1969}. In either case, comparing misfit texts
results in more judgment discrepancies, yet the BTL model neither
accounts for these cases nor offers any means to address them.
...
features, such as writing style or general quality. Although the CJ
literature acknowledges the existence of such hierarchical structures,
the statistical approaches to account for this additional source of
dependence have been insufficient. For instance, when CJ data
...
\citep{Kline_et_al_2023, Hoyle_et_al_2023}. Furthermore, the reduced
precision in hypothesis tests diminishes their statistical power,
increasing the likelihood of committing type-I or type-II errors
...
fundamental building blocks \citep{Neal_2020, McElreath_2020}. This
...
\((O^{cp}_{haibjk})\) and several related variables. The subscripts
...
different texts written by the same student \((h = i;\) \(a \neq b)\)
and between texts written by distinct students \((h \neq i;\)
\(a = b \; \text{is permitted})\), each judged once or repeatedly by all
judges \((j = 1,\dots,n_{J};\) \(k = 1,\dots,n_K)\). However, it
excludes cases where a judge compares a student's text to itself,
whether once or multiple times \((h = i;\) \(a = b;\)
...
difference \((D_{haibjk})\) (see Section~\ref{sec-thurstone_theory}).
...
\(V = \{ O_{haibjk}, D_{haibjk}, T_{ha}, T_{ib}, B_{kj} \}\). Finally,
...
\(X_{I}\). This branch also shows how \(T_{I}\), along with
\emph{relevant} \footnote{refer to footnote 2.} text-related variables
\((X_{IA})\) (e.g., text length), and texts' idiosyncratic errors
\(e_{IA}\) causally influence the texts' writing-quality trait
\((T_{IA})\), the first primary latent variable of interest. The error
term \(e_{IA}\) captures variations in the texts' traits that remain
unexplained by \(T_{I}\) or \(X_{IA}\). In this model, \(X_{I}\) is an
observed matrix with \(n_{I}\) rows and \(q_{I}\) columns (variables),
and both \(e_{I}\) and \(T_{I}\) are latent vectors of length \(n_{I}\).
Similarly, \(X_{IA}\) is an observed matrix with dimensions
...
branch also shows how \(B_{J}\), along with \emph{relevant} \footnote{refer
  to footnote 2.} judgment-related variables \((Z_{JK})\) (e.g., the
...
student and the third judge. They also omit the second text from each
individual and the third repeated judgment from each judge. Given
...
of \(S_{JK}\). Each element \(s_{(s)}\) is a binary value indicating the
presence or absence of data rows in the matrix \(R\) resulting from the
sample mechanism (see equation (\ref{eq-mat26})). Thus, the vector
contains \(n\) elements, matching the number of rows in \(R\), and the
sum of its elements represents the total data sample:
...
Here, \emph{conditioning} means restricting the statistical analysis to
a specific subset of the population based on the values of a conditioned
variable \citep[pp.~32]{Neal_2020}. Essentially, \(S\) serves as the
sample equivalent for the \(R\) matrix, that is, this vector selects
\emph{all repeated judgments a subset of judges makes for the subset of
stimuli produced by the sampled individuals}. This process ultimately
generates the sample-comparison outcome \(O^{sc}_{R}\) from the
...
exclusively to Simple Random Sampling (SRS) designs
\citep{Kohler_et_al_2019}. In these designs, each repeated judgment,
judge, stimulus, and individual has the same probability of being
included in the sample as any other observation within their respective
...
practical feasibility of the comparison task \citep{Boonen_et_al_2020}--
a realistic scenario must also account for judges comparing only
specific stimuli from certain individuals.
...
in the \(R\) matrix, with each element \(c_{(c)}\) being a binary value
indicating the presence or absence of data rows in the \(R\) similar to
equation (\ref{eq-mat26}). This definition assumes that the subset of
judges \emph{does not} perform all repeated judgments for the subset of
stimuli produced by the sampled individuals. Instead, they conduct a
``sufficient number of comparisons'' \(n_{C}\) to ensure the accurate
estimation of the proportion \(P(B>A)\) for each pair of stimuli
\citep[pp.~267]{Thurstone_1927b}.

DAG~\ref{fig-cj14_dag} also incorporates the comparison mechanism \(C\)
into the conceptual-population model. The DAG represents the
...
comparison mechanism. Moreover, it represents the \emph{comparison
mechanism} \(C\) as a causal factor influencing the final comparative
outcome \(O_{R}\). Equivalent to the sampling mechanism, a square
encloses this vector, indicating that \(C\) is a conditioned variable
that determines \emph{which repeated judgments the subset of judges
makes for the subset of stimuli produced by the sampled individuals}.
This process ultimately derives the final comparative outcome \(O_{R}\)
from the sample-comparison outcome \(O^{sc}_{R}\).
...
Random Allocation Comparative Designs \citep{Bramley_2015} or Incomplete
Block Designs \citep{Lawson_2015}. In these designs, every repeated
...
applies to this outcome \(O_{R}\). This assumption removes the necessity
of describing the unobserved outcomes \(O^{cp}_{R}\) and \(O^{sc}_{R}\).
...
Using SCM~\ref{fig-cj15_scm} we finally define the

\(\sum_{g=1}^{G_{XI}} s_{XI}/G_{XI} = 1\)
\(\sum_{g=1}^{G_{ZJ}} s_{ZJ}/G_{ZJ} = 1\)
