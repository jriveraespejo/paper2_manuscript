...
non-parametric functions of other variables.

For instance, Figure~\ref{fig-CJ1_scm} display the SCM relating the CJ
outcome \((O_{kjabi_{1}i_{2}})\), discriminal difference
\((D_{kjabi_{1}i_{2}})\), perceived discriminal processes for each text
within each student \((PT_{kjai_{1}}, PT_{kjbi_{2}})\), the ``true''
discriminal processes for each text within each student
\((T_{kjai_{1}}, T_{kjbi_{2}})\), and the judges' biases \((B_{kj})\).
Notice that the structural equations use the symbol `\(:=\)' to denote
