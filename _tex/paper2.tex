...
\includegraphics[width=0.23\linewidth,height=\textheight,keepaspectratio]{./images/png/CJ_TM_01.png}
...
\includegraphics[width=0.32\linewidth,height=\textheight,keepaspectratio]{./images/png/CJ_TM_02.png}
...
\includegraphics[width=0.45\linewidth,height=\textheight,keepaspectratio]{./images/png/CJ_TM_03.png}
...
\includegraphics[width=0.59\linewidth,height=\textheight,keepaspectratio]{./images/png/CJ_TM_06.png}

}

\caption{\label{fig-CJ_TM_06}}
...
\includegraphics[width=0.68\linewidth,height=\textheight,keepaspectratio]{./images/png/CJ_TM_07.png}

}

\caption{\label{fig-CJ_TM_07}}
...
\includegraphics[width=0.68\linewidth,height=\textheight,keepaspectratio]{./images/png/CJ_TM_08.png}

}

\caption{\label{fig-CJ_TM_08}}
...
\includegraphics[width=0.68\linewidth,height=\textheight,keepaspectratio]{./images/png/CJ_TM_09.png}

}

\caption{\label{fig-CJ_TM_09}}
...
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{./images/png/CJ_TM_11.png}

}

\caption{\label{fig-CJ_TM_11}}
...
\includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{./images/png/CJ_TM_13.png}

}

\caption{\label{fig-CJ_TM_13}}
...
variable for each student's potential outcomes. In contrast,
\emph{conditioning} involves restricting the focus to the subset of the
population defined by the conditioning variable
\citep[pp.~32]{Neal_2020} (see Equation~\ref{eq-group_diff}).\\

Note that if a student is assigned to intervention \(T_{i}=1\), the
potential outcome under \(T_{i}=2\) becomes a counterfactual, as it is
no longer observed nor observable. To address this challenge, the
structural approach extends the ICE to the \emph{average causal effect}
(ACE, Equation~\ref{eq-ACE}), representing the average difference
between observed potential outcomes and their counterfactual
counterparts.
...
\emph{backdoor criterion} \citep[pp 37]{Neal_2020}.
...
firt identify the \emph{conditional average causal effect} (CACE,
...
Equation~\ref{eq-mACE}. Here, \(E_{X}[\cdot]\) represents the marginal
expected value over \(X\) \citep{Morgan_et_al_2014}.

\begin{equation}\phantomsection\label{eq-mACE}{
\begin{aligned}
...
connecting. SCM \ref{fig-scm_bb3} and DAG \ref{fig-mdag_bb3} depict a
\emph{chain}, where \(X_{1}\) influences \(X_{2}\), and \(X_{2}\)
...
\subsection{The probabilistic implications of SCMs and
...
Section~\ref{sec-appendix-C} clearly illustrate which (in)dependencies
...
supports the theoretical assumption that the observed variables
\(X_{1}\) and \(X_{3}\) are unconditionally independent or d-separated
...
the structural equations illustrated in Section~\ref{sec-appendix-C} and
the CPDs.
...
and colliders. These (in)dependencies apply to any data generated by
these structures. For instance, applying the BNF to the chain structure
depicted in the SCM \ref{fig-scm_bb3} and DAG \ref{fig-mdag_bb3} allow
researchers to represent the joint distribution for the observed
...
Researchers can revisit the scenario in Section~\ref{sec-appendix-B}
using these insights. By applying the BNF to the SCM
\ref{fig-scm_example1} and DAG \ref{fig-mdag_example1a} structures, they
can represent the joint probability of the observed variables as
\(P(Y, T, X) = P(Y \:|\:T, X) P(T \:|\:X) P(X)\). Assuming the
\emph{direct effect} of \(T\) on \(Y\) (\(T \rightarrow Y\), black path)
is absent, researchers can infer from the joint probability that \(Y\)
is not unconditionally independent of \(T\) \((Y \:\bot\:T)\). This
dependency arises because a non-causal path exists between \(T\) and
\(Y\) through \(X\) (\(T \leftarrow X \rightarrow Y\), gray path),
indicating that \(X\) is a confounder. However, researchers can observe
that conditioning on \(X\), depicted as a square in DAG
\ref{fig-mdag_example1a}, results in \(Y \:\bot\:T \:|\:X\), eliminating
the confounding effect and allowing the identification of the direct
effect \(T \rightarrow Y\) if it exists.
...
\subcaption{\label{fig-mdag_example1a}Conditioned DAG}
...
\subsection{From probability to causality}\label{sec-appendix-E}

In addition to the assumptions required to encode probabilistic
information, SCMs and DAGs rely on the modularity assumption to
represent interventions \(P(Y_{i} \:|\:do(T_{i}=t))\) (see
Section~\ref{sec-appendix-B}). Defining these interventions enables
researchers to identify the ACE from associational quantities through
statistical conditioning, effectively disentangling causation from mere
association.

The modularity assumption underpins the concepts of Truncated
Factorization and manipulated graphs, which are essential for
representing interventions within SCMs and DAGs. The \emph{modularity
assumption} posits that intervening on a node modifies only the causal
mechanism of that node, leaving others unchanged
\citep[pp.~34]{Neal_2020}. Building on this principle, the
\emph{Truncated Factorization} (TF) simulates physical interventions by
removing specific functions from the conceptual model and substituting
them with constants, preserving the rest of the structure unchanged
{[}Pearl\_2010{]}. The probabilistic implications of this factorization
are formalized in Equation~\ref{eq-trunc_factor}, where \(S\) denotes
the subset of variables \(X_{p}\) directly influenced by the
intervention. Complementing the TF, \emph{manipulated graphs} are DAGs
where specific edges have been removed to reflect the effects of
interventions \citep[pp.~34]{Neal_2020}.
...
Using the TF, researchers can define the \emph{backdoor adjustment}, a
method for identifying causal effects. This adjustment specifies that if
a variable \(X_{p} \in S\) serves as a \emph{sufficient adjustment set}
for the effect of \(X_{a}\) on \(X_{b}\), then the ACE can be identified
using Equation~\ref{eq-backdoor} (see Section~\ref{sec-appendix-B}).

\begin{equation}\phantomsection\label{eq-backdoor}{
P(X_{a} \:|\:do(X_{b}=x)) = \sum_{Xp} P(X_{a} \:|\:X_{b}=x, X_{p}) P(X_{p})
}\end{equation}

Ultimately, backdoor adjustment enables researchers to express the ACE
as:

\begin{equation}\phantomsection\label{eq-backdoor_adjustment}{
\begin{aligned}
\tau &= E[\tau_{i}] \\
  & = E[X_{a} \:|\:do(X_{b}=1)]- E[X_{a} \:|\:do(X_{b}=2)] \\
  & = \sum_{Xp} X_{a} \cdot P(X_{a} \:|\:X_{b}=1, X_{p}) P(X_{p}) - \sum_{Xp} X_{a} \cdot P(X_{a} \:|\:X_{b}=2, X_{p}) P(X_{p})
\end{aligned}
}\end{equation}
...
\subcaption{\label{fig-mdag_example1b}Manipulated DAG}
...
\subsection{The estimation process}\label{sec-appendix-F}
...
likelihood of the data, \(P(X_{1}, X_{2}, \dots, X_{P} \:|\:\theta)\),
and the prior distribution, \(P(\theta)\) \citep{Everitt_et_al_2010},
where \(X_{P}\) represents a random variable, and \(\theta\) represents
a one-dimensional parameter space for simplicity. After observing
