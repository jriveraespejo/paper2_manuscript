...
issues by extending Thurstone's model through a systematic and
...
in detail. Section~\ref{sec-theoretical} extends Thurstone's model to
address these challenges. The model integrates the theoretical core
...
each receiving a different teaching method. Judges would then compare
pairs of students' written texts, producing a dichotomous outcome that
reflects the relative quality of each text. In this scenario,
researchers can reformulate the research question as the estimand:
...
However, Section~\ref{sec-theory-issues} presents strong evidence that
the BTL model has several statistical and measurement limitations. These
limitations hinder the model's ability to identify distinct estimands
relevant to CJ inquiries, including the one presented in the previous
paragraph. Identification is crucial because it is a necessary condition
for ensuring consistent estimators. Without identification, researchers
cannot achieve consistency even with ``infinite'' and error-free data.
In other words, the estimates derived from the estimator fail to
converge to the ``true'' value of the estimand as the data size
approaches infinity \citep{Everitt_et_al_2010}. As a result, deriving
...
extending Thurstone's model through a systematic and integrated approach
based on the structural approach to causal inference. Specifically, it
...
relevant to CJ experiments. These features include the selection of
judges, stimuli, and comparisons. Combining these components clarifies
the interactions among all actors and processes involved in CJ
experiments. Accordingly, Section~\ref{sec-theory-theoretical_P}
incorporates the theoretical principles into a
\emph{conceptual-population model}. Meanwhile,
Section~\ref{sec-theory-theoretical_SC} integrates the assessment design
features into a \emph{sample-comparison model}.
...
For instance, following the scenario outline in
Section~\ref{sec-theoretical}, Figure~\ref{fig-cj04_scm} displays the
SCM that relates the outcome of the CJ experiment \((O_{abhijk})\), to
several endogenous variables \(V\). These variables include the
discriminal difference \((D_{abhijk})\), perceived discriminal processes
\((PT_{ahjk}, PT_{bijk})\), the ``true'' discriminal processes
\((T_{ahjk}, T_{bijk})\), and the judges' biases \((B_{kj})\), formally
expressed as
\(V = \{D_{abhijk}, PT_{ahjk}, PT_{bijk}, T_{ahjk}, T_{bijk}, B_{kj}\}\).
The structural equations \(\{f_{O},f_{D},f_{PT}\}\) define the
dependencies among these variables. Notably, in this scenario, the
judges produce a dichotomous outcome \((O_{abhijk}) = \{0,1\}\).
Moreover, the indices \(\{a,b\}\) represent the compared texts,
\(\{h,i\}\) label the students who wrote these texts, \(j\) denotes the
judge, and \(k\) represents the comparison number. Regarding the \(k\)
index, the index accounts for cases in which judges perform multiple
comparisons of the same texts written by the same students, as the one
discussed in Section~\ref{sec-theoretical}.
...
Because it quickly becomes intractable, and without loosing generality,
the (latent) ``perceived'' and ``true'' discriminal processes for the
stimuli can be depicted in a vector for each judge, as in
