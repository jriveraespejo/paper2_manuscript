...
the conceptual-population model then includes two hierarchical
structural components. Each component defines how different observed or
latent variables influence the primary latent variable of interest
...
components in the motivating example. Specifically, the top branch of
...
of \emph{relevant} text-related variables \(X_{IA}\) causally influence
the texts' written-quality trait \(T_{IA}\). The DAG also highlights the
...
A cautious reader will notice that these descriptions rely on several
relevant variables. In this study, a \emph{relevant} or a
\emph{sufficient adjustment} set of variables (potentially an empty set)
refers to those that satisfy the \emph{backdoor criterion}
\citep[pp.~37]{Neal_2020}, that is, variables that block all non-causal
paths without introducing new ones
\citep{Pearl_2009, Pearl_et_al_2016, Morgan_et_al_2014}.
