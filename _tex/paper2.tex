...
in the statistical analysis of CJ data can compromise the reliability of
CJ's trait estimates. This overreliance, coupled with the apparent
disconnect between CJ's approach to trait measurement and hypothesis
testing, may also undermine the validity of these estimates
\citep[pp.~2]{Perron_et_al_2015}. Thus, addressing these issues through
a more systematic and integrated approach could provide significant
statistical and measurement benefits. For this reason, the study employs
the structural approach to causal inference, as it fulfills these
requirements.
...
A motivating example helps to clarify these concepts. The example will
appear throughout the document to illustrate various aspects of the
proposed extension. In this scenario, researchers aim to determine: ``To
...
two groups, each receiving a different teaching method. Judges would
then compare pairs of students' written texts (stimuli), producing a
dichotomous outcome that reflects the relative quality of each text
(trait). Based on this setup, researchers could reformulate the research
...
of students?''. Following current CJ practices, researchers would then
rely on estimates from the BTL model, or its transformations, to
...
estimands relevant to CJ inquiries, including the one from the
motivating example. Identification is crucial because it is a necessary
condition for ensuring consistent estimators. Without identification,
\emph{consistency} --the property of an estimator whose estimates
converge to the ``true'' value of the estimand as the data size
approaches infinity \citep{Everitt_et_al_2010}-- cannot be achieved,
even with ``infinite'' and error-free data. As a result, deriving
meaningful insights from finite data also becomes unattainable
...
As a result, this section addresses the identified limitations by
extending Thurstone's general form (see Table~\ref{tbl-thurstone_cases})
through a systematic and integrated approach based on the structural
approach to causal inference. Specifically, it leverages the
capabilities of this approach to formalize the combination of the core
theoretical principles outlined in Section~\ref{sec-thurstone_theory}
with key assessment design features relevant to CJ experiments, such as
the selection of judges, stimuli, and comparisons.

In addition to addressing the identified limitations, the extended model
...
the current data analysis paradigm from passively accepting CJ's model
assumptions to actively testing them with the specific data under
analysis. For more details on these assumptions, refer to
Section~\ref{sec-theory-issues}.

Accordingly, Section~\ref{sec-theory-theoretical_P} incorporates the
theoretical principles into what the study calls the
\emph{conceptual-population model}. This model assumes that researchers
have access to a conceptual population of comparison data, meaning data
representing all repeated comparisons made by every available judge for
each pair of stimuli produced by each pair of individuals in the
population, thus its name. Conversely,
Section~\ref{sec-theory-theoretical_SC} integrates the assessment design
features into what the study calls the \emph{sample-comparison model}.
...
\emph{conceptual-population model}, the study defines SCMs, which are
formal mathematical models characterized by a set of \emph{endogenous}
...
motivating example. This SCM maps the relationship between the outcome
of the CJ experiment \((O_{abhijk})\) to several other variables. In
this model, the indices \(\{a,b\}\) represent the compared texts
(stimuli), with \(a \neq b\). The indices \(\{h,i\}\) label the students
who wrote the texts (individuals), where \(h = i\) is possible. The
index \(j\) denotes the judge, and \(k\) denotes the comparison number,
which accounts for experimental cases where judges perform multiple
comparisons of the same texts written by the same students, i.e., a
\emph{repeated measures design} \citep[pp.~366-376]{Lawson_2015}.
Finally, the SCM presents the preliminary set of structural equations
\(F = \{f_{O},f_{D},f_{PT}\}\), which define the non-parametric
dependencies among these variables.

Note that the SCM~\ref{fig-cj04_scm} incorporates not only the variables
described by Thurstone's theory, such as the discriminal difference
\((D_{abhijk})\) and the texts' \emph{true} discriminal processes
\((T_{ahjk}, T_{bijk})\) (refer to Section~\ref{sec-thurstone_theory}),
but also innovates by incorporating additional variables central to CJ
experiments, such as the texts' \emph{perceived} discriminal processes
\((PT_{ahjk}, PT_{bijk})\) and the judges' perceptions \((B_{kj})\).
Together with the outcome, these variables constitute the preliminary
set of endogenous variables
\(V = \{O_{abhijk}, D_{abhijk}, PT_{ahjk}, PT_{bijk}, T_{ahjk}, T_{bijk}, B_{kj}\}\).

By incorporating the texts' perceived discriminal processes into the
conceptual-population model, this study considers a crucial aspect of CJ
experiments that previous literature has overlooked. It recognizes that
the texts' true discriminal processes become an observable outcome only
through the judges' perceptions. In other words, researchers observe the
CJ outcome only when both variables create a \emph{pure interaction
effect}, meaning that neither variable has a direct effect on the
outcome, but their interaction produces an effect
\citep{Attia_et_al_2022}.

Conversely, based on the evidence presented in
Section~\ref{sec-theory-issue1b}, the study assumes that other relevant
stimulus characteristics influence the judges' perceptions, leading to
biases that resist cancellation and ultimately introduce dependencies
between the stimuli. To address this, the study incorporates the judges'
perceptions into the conceptual-population model, following the
...
SCM~\ref{fig-cj04_scm}. The graph shows the direction of causal
...
endogenous variables, such as the outcome \(O_{abhijk}\), and variables
considered latent according to the theory and practice of CJ, such as
the discriminal difference, the perceived discriminal processes, the
true discriminal processes, and the judges' perceptions.

It is easy to intuit that describing the CJ system by assigning an index
and a variable to each discriminal process could quickly become
intractable and cumbersome due to the large size of the DAG and the
repetition of its components. Therefore, to maintain generality, the
study groups pairs of discriminal processes with different indices into
matrices that contain these pairs. First, the study defines the vectors
\(U_{A} \in \mathbb{N}^{A}\), \(U_{I} \in \mathbb{N}^{I}\) containing
the indices of the stimuli and individuals, where:

\[
U_{A} = [1, \dots, a, \dots, b, \dots, A] ;\quad U_{I} = [1, \dots, a, \dots, b, \dots, I]
\]

Then, the study defines the matrices of stimuli and individual
comparisons, \(P \in \mathbb{N}^{m \times 2}\) and
\(R \in \mathbb{N}^{n \times 2}\), respectively. Here, each row of \(P\)
represents a pairing \(\{a,b\}\) of stimuli indices and each row of
\(R\) represents a pairing \(\{h,i\}\) of individual indices, i.e.,

\[
P = \{(a, b) \mid a,b \in U_{A}, a \neq b\} ;\quad R = \{(h, i) \mid h,i \in U_{I}\}
\]

Here, \(m\) and \(n\) represents the number of unique pairwise
comparisons between stimuli, and unique pairwise comparison per
individuals, which corresponds to the number of ways to choose two
elements from \(U_{A}\) and \(U_{I}\), typically given by:
...
\subcaption{\label{fig-cj05_scm}SCM}
...
\subcaption{\label{fig-cj05_dag}DAG}

\end{minipage}%

\caption{\label{fig-cj05}CJ's outcome, discriminal difference,
discriminal processes and judges' biases}
...
\subcaption{\label{fig-cj10_scm}SCM}
...
\subcaption{\label{fig-cj10_dag}DAG}

\end{minipage}%

\caption{\label{fig-cj10}CJ's outcome, discriminal difference,
discriminal processes and judges' biases}
...
\caption{\label{fig-cj12}CJ's outcome, discriminal difference,
discriminal processes and judges' biases}
...
\caption{\label{fig-cj14}CJ's outcome, discriminal difference,
discriminal processes and judges' biases}
...
\caption{\label{fig-cj15}CJ's outcome, discriminal difference,
discriminal processes and judges' biases}
