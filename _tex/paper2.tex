...
Nevertheless, despite the increasing number of studies on CJ, research
in this domain remains unsystematic and fragmented, leaving several
critical issues unresolved. This study addresses two prominent issues in
CJ literature that can undermine the reliability and validity of CJ's
trait estimates. The first problem concerns the overreliance on
Thurstone's Case V assumptions in the statistical analysis of CJ data.
The second involves the apparent disconnect between CJ's approach to
trait measurement and hypothesis testing.

The study is organized into six main sections. An overview of
Thurstone's theory is presented in Section~\ref{sec-thurstone_theory},
followed by a detailed examination of the identified issues in
Section~\ref{sec-theory-issues}. In Section~\ref{sec-theoretical}, the
study proposes a theoretical model for CJ that addresses these
challenges. The model incorporates Thurstone's core principles alongside
key assessment design features relevant to CJ experiments, such as
judges, stimuli, and comparison selections. In
Section~\ref{sec-statistical}, these practical and theoretical
considerations are translated into a statistical model for the analysis
of pairwise comparison data. Finally, Section~\ref{sec-discussion}
explores the implications of the study's findings and the avenues for
future exploration, and Section~\ref{sec-conclusion} provides a summary
of the key insights.
...
This section discusses the two prominent issues in CJ literature that
can undermine the reliability and validity of CJ's trait estimates.
...
In aggregate, researchers' overreliance on Thurstone's Case V
assumptions in the statistical analysis of CJ data can compromise the
reliability of CJ's trait estimates. This overreliance, coupled with the
apparent disconnect between CJ's approach to trait measurement and
hypothesis testing, may also undermine the validity of these estimates
\citep[pp.~2]{Perron_et_al_2015}. Consequently, addressing these issues
and examining the factors influencing CJ experiments by adopting a more
systematic and integrated approach could offer several statistical and
...
Identification is essential because it is a necessary condition for
\emph{consistency}. An estimator is \emph{consistent} if it converges to
the ``true'' value of an estimand as the sample size approaches infinity
\citep{Everitt_et_al_2010}. Without identification, an estimator cannot
achieve consistency, rendering it incapable of reliably computing the
estimand, even with ``infinite'' and error-free data. Consequently,
deriving meaningful insights about the estimand from finite data becomes
impossible \citep[pp.~5]{Schuessler_et_al_2023}. Therefore, to ensure
accurate and reliable estimates, researchers prioritize estimators with
desirable identification properties. For instance, the Z-test is a
widely used estimator for comparing group proportions, yielding accurate
estimates when its underlying assumptions are satisfied
\citep{Kanji_2006}. Furthermore, researchers can interpret estimates
from the Z-test as causal, provided the data is collected through a
