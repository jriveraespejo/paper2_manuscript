...
conceptual-population outcome \((O^{cp}_{abhijk})\) and several related
variables. The subscripts \(a\) and \(b\) represent the compared texts
(stimuli), while the indices \(h\) and \(i\) label the students who
authored them (individuals). Moreover, the index \(j\) refers to the
judge, and \(k\) specifies the judgment number, accounting for
experimental conditions where judges compare the same texts written by
the same students multiple times, i.e., a \emph{repeated measures
design} \citep[pp.~366-376]{Lawson_2015}. Notably, the indexing system
allow comparisons between different texts written by the same student
...
as such a comparison lacks practical meaning within the CJ framework.

The other variables present in SCM~\ref{fig-cj03_scm} include the texts'
discriminal processes \((T_{ahjk}, T_{bijk})\) and their discriminal
difference \((D_{abhijk})\), in line with Thurstone's theory (see
Section~\ref{sec-thurstone_theory}). Additionally, based on the
...
the conceptual-population model incorporates the judges' biases
...
\citep{McElreath_2020}. DAGs depict observed variables as solid black
circles and unobserved (latent) variables as open circles
\citep{Morgan_et_al_2014}. The \emph{standard representation} of DAGs
typically omits exogenous variables for simplicity, while the
\emph{magnified representation} includes them to highlight potential
issues related to conditioning and confounding
\citep{Cinelli_et_al_2020}. This section adopts the magnified
representation to enhance clarity.
...
theory and the practical aspects of CJ's experiments. It also
...
Although specifying a system's data structure is not necessary
requirement for using SCMs and DAGs, defining one can improve clarity
and simplify their notation. Thus, we define specific data structures to
facilitate the description of the CJ system. First, we define the
...
where each row of \(IA\) corresponds to a unique pairing of individuals
\(h, i\) with stimuli \(a, b\). Here, \(n_{I}\) represents the number of
individuals, while \(n_{A}\) denotes the number of stimuli available per
individual. Thus, the matrix \(IA\) is composed of natural numbers
\(\mathbb{N}\), with \((n_{I} \cdot n_{A})\) rows and \(2\) columns.
Similarly, each row of \(JK\) specifies a unique pairing of a judge
\(j\) with a judgment \(k\). In this case, \(n_{J}\) indicates the total
number of judges, and \(n_{K}\) represents the number of judgments each
judge performs. Therefore, the matrix \(JK\) is also composed of natural
numbers \(\mathbb{N},\) with \((n_{J} \cdot n_{K})\) rows and \(2\)
columns.

Importantly, since the discriminal difference of the stimuli becomes an
observable outcome only through judges' perceptions, it is necessary to
define the matrix \(RPUV\), where each row of the matrix uniquely pairs
the corresponding rows from the matrices \(IA\) and \(JK\) as follows:
...
The \(RPUV\) matrix is also composed of natural numbers \(\mathbb{N}\),
and has a dimension of
\({n_{I} \cdot n_{A} \choose 2} \cdot n_{J} \cdot n_{K}\) rows and \(6\)
columns. The dimension of the matrix results from the
conceptual-population model's assumption that researchers have access to
data representing all repeated judgments made by every available judge
for each pair of stimuli produced by each pair of individuals in the
population. Here, the term \({n_{I} \cdot n_{A} \choose 2}\) represents
the Binomial coefficient, which quantifies the total number of
individual-stimuli comparison pairs in the conceptual population. The
...
Readers can better visualize the structure of these matrices by
considering relatively small values for the parameter corresponding to
number of individuals \((n_{I} = 5)\), the number of available stimuli
per individual \((n_{A} = 2)\), the total number of judges
\((n_{J} = 3)\), and the number of judgments each judge performs
\((n_{K} = 3)\). With these values, the matrices described in equations
(\ref{eq-mat12}) and (\ref{eq-mat3}) are represented as follows:
...
Thus, with the data structure defined by the matrices in equations
(\ref{eq-mat12}) and (\ref{eq-mat3}), we can now re-express the
SCM~\ref{fig-cj03_scm} and DAG~\ref{fig-cj03_dag} in an equivalent
vectorized form, as shown in Figure~\ref{fig-cj04}. In this form, the
outcome \(O^{cp}_{RPUV}\), the texts' discriminal difference
\(D_{RPUV}\), their discriminal processes \(T_{IA}\), and the judges'
biases \(B_{JK}\) are no longer scalar values but are instead vectors.
These vectors encompass all the values from the conceptual population.
Specifically, \(O^{cp}_{RPUV}\) and \(D_{RPUV}\) are vectors with a
dimension of \({n_{I} \cdot n_{A} \choose 2} \cdot n_{J} \cdot n_{K}\)
values each, while \(T_{IA}\) and \(B_{JK}\) have dimensions of
\((n_{I} \cdot n_{A})\) and \((n_{J} \cdot n_{K})\) values,
respectively.
...
Finally, building on the arguments developed in
...
components. Each component defines how different manifest or latent
variables influence the primary latent variable of interest
\citep{Everitt_et_al_2010}. This addition enables analyses that
simultaneously account for the traits and their associated
uncertainties, linking the trait measurement process with hypothesis
testing, thereby reducing bias and enhancing the measurement precision.

Figure~\ref{fig-cj09} displays the two (hierarchical) structural
components in the motivating example. Specifically, the
DAG~\ref{fig-cj09_dag} shows that the first component describes how the
students' overall writing quality trait \(T_{I}\) and a set of
...
quality that neither the students' traits nor other relevant
text-related variables can explain. In this description, \(T_{I}\) is a
vectors of dimension \(n_{I}\), \(e_{IA}\) is a vector of dimension
\((n_{I} \cdot n_{A})\), while \(X_{IA}\) is a matrix with
\((n_{I} \cdot n_{A})\) rows and \(p_{IA}\) columns.

At a higher level in the hierarchy, the DAG~\ref{fig-cj09_dag} also
describes how a set of \emph{relevant} student-related variables
\(X_{I}\) causally influence the students' overall writing quality trait
\(T_{I}\). Additionally, the DAG accounts for idiosyncratic errors
\(e_{I}\) that capture variations in the students' writing quality that
the relevant student-related variables cannot explain. Here, \(e_{I}\)
is a vector of dimension \(n_{I}\), and \(X_{I}\) is a matrix with
\(n_{I}\) rows and \(p_{I}\) columns.

Similarly, the DAG~\ref{fig-cj09_dag} shows that the second component
describes how the judges' overall bias \(B_{J}\) and a set of
\emph{relevant} judgment-related variables \(Z_{JK}\) causally influence
the biases associated with each text \(B_{JK}\). The DAG also highlights
the idiosyncratic errors \(e_{JK}\) that capture variations in the
texts' biases that neither the judges' bias nor other relevant
judgment-related variables can explain. In this description, \(B_{J}\)
is a vectors of dimension \(n_{J}\), \(e_{JK}\) is a vector of dimension
\((n_{J} \cdot n_{K})\), while \(Z_{JK}\) is a matrix with
\((n_{J} \cdot n_{K})\) rows and \(p_{JK}\) columns.

Furthermore, at a higher level, the DAG~\ref{fig-cj09_dag} describes how
a set of \emph{relevant} judge-related variables \(Z_{J}\) causally
influence the judges' overall bias \(B_{J}\). The DAG also accounts for
idiosyncratic errors \(e_{J}\), which capture the variations in the
judges' biases that the relevant judge-related variables cannot explain.
Here, \(e_{J}\) is a vector of dimension \(n_{J}\), and \(Z_{J}\) is a
matrix with \(n_{J}\) rows and \(p_{J}\) columns.
...
sets of relevant variables. In this study, a \emph{relevant} or a
\emph{sufficient adjustment} set of variables (potentially an empty set)
refer to those that satisfy the \emph{backdoor criterion}
...
\citet[pp.~150]{Kohler_et_al_2019} emphasized, drawing conclusions about
the real world from observed data inevitably requires assumptions, and
this requirement holds true for both observational and experimental
data.
