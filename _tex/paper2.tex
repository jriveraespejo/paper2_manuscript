...
produce high-quality written texts?'' To investigate this, they design a
CJ experiment by randomly assigning students (individuals) to two
groups, each receiving a different teaching method. Judges then compare
pairs of students' written texts (stimuli), producing a dichotomous
outcome that reflects the relative quality of each text (trait). Based
on this setup, researchers could reformulate the research question as
the estimand: ``\emph{On average}, is there a difference in the ability
to produce high-quality written texts between the two groups of
students?''. Following current CJ practices, researchers would then rely
on estimates from the BTL model, or its transformations, to approximate
this estimand.
...
Thus, this section addresses the identified limitations by extending
Thurstone's general form (see Table~\ref{tbl-thurstone_cases}) through a
systematic and integrated approach based on the structural approach to
causal inference. Specifically, it leverages the capabilities of this
approach to formalize the combination of the core theoretical principles
outlined in Section~\ref{sec-thurstone_theory} with key assessment
design features relevant to CJ experiments, such as the selection of
judges, stimuli, and comparisons.

In addition to overcoming the identified limitations, the extended model
...
the model assumptions to actively testing if those assumptions fit the
specific data under analysis. For more details on these assumptions,
refer to Section~\ref{sec-theory-issues}.
...
\emph{conceptual-population model}, the study defines SCMs. SCMs are
