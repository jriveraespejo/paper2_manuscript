...
transformation
\citetext{\citealp[pp.~16]{vanderLinden_et_al_2017_I}; \citealp{McElreath_2022}}.
...
trait and place the compared stimuli along its continuum
\citep[pp.~269]{Thurstone_1927b}. Additionally, the CJ literature shows
that researchers frequently use point estimates of BTL scores or their
transformations to conduct further analyses or hypothesis tests. For
example, researchers have used these scores to identify `misfit' judges
and stimuli
...
neglecting measurement error in an outcome can reduce the precision of a
predictor' estimated effect.
...
produce high-quality written texts?'' To investigate this, a researcher
designs a CJ experiment by randomly assigning students (individuals) to
...
dichotomous outcome reflecting the relative quality of each text
(trait). Based on this setup, researchers can reformulate the research
...
of students?''. Following current CJ practices, researchers then rely on
...
have access to a ``conceptual population'' of comparative data, that is,
...
population. Conversely, Section~\ref{sec-theory-theoretical_SC}
...
Moreover, Figure~\ref{fig-cj03_dag} displays the DAG corresponding to
SCM~\ref{fig-cj03_scm}, illustrating the expected causal outlined in
Thurstone's theory. The graph shows that the discriminal processes of
the texts \((T_{ha}, T_{ib})\) influence their discriminal difference
\((D_{hiabjk})\), which in turn determines the outcome
...
outcome, and latent endogenous variables, including the texts'
discriminal processes, their discriminal difference, and the judges'
biases.

\subsubsection{The conceptual-population data
...
system outlined in the previous section into an equivalent vectorized
form, we first define the vectors \(I\) and \(J\), along with the
...
where \(n_{K}\) indicates the number of judgments each judge makes.
Consequently, the matrix \(JK\) has \((n_{J} \cdot n_{K})\) rows and
\(2\) columns.

Additionally, we define the matrix \(RPUV\) that assigns each row from
the \(IA\) matrix to a corresponding row from the \(JK\) matrix. Thus,
the matrix consists of
\(n = {n_{I} \cdot n_{A} \choose 2} \cdot n_{J} \cdot n_{K}\) rows and
\(6\) columns. Here, the term \({n_{I} \cdot n_{A} \choose 2}\)
represents the Binomial coefficient, which quantifies the total number
of repeated judgments each judge makes for every pair of stimuli
generated by each pair of individuals in the population. The matrix is
defined as follows:
...
matrices described in equations (\ref{eq-mat12}) and (\ref{eq-mat3})
...
Then, using equations (\ref{eq-mat12}) and (\ref{eq-mat3}), we can
...
values. These vectors capture all the observations from the conceptual
population. Specifically, \(O^{cp}_{RPUV}\) is an observed vector of
length \(n={n_{I} \cdot n_{A} \choose 2} \cdot n_{J} \cdot n_{K}\), and
\(D_{RPUV}\) is a latent vector of the same dimension. Moreover,
...
structural components}. A structural component describes how different
observed or latent variables influence the primary latent variable of
interest \citep{Everitt_et_al_2010}. Moreover, their hierarchical nature
enable analyses when the stimuli exhibit a hierarchical structure (refer
to Section~\ref{sec-theory-issue1b}) and when researchers aim to test
hypotheses while accounting for the uncertainties associated with the
traits (see Section~\ref{sec-theory-issue2}).

Figure~\ref{fig-cj09} displays these components applied to examine the
effect of different teaching methods on students' writing ability. The
top branch of DAG~\ref{fig-cj09_dag} depicts the first component. This
...
assumption that researchers have access to a \emph{conceptual
population} of comparative data, encompassing all repeated judgments
from every judge for each pair of stimuli produced by every individual
in the population. Since researchers rarely meet this assumption in
practice, they must consider a more realistic scenario. The extensions
to the conceptual-population model, derived from this realistic
scenario, are concisely described in the sample-comparison model.
...
Following the convention of \citet[pp.~499-516]{McElreath_2020} and
\citet[pp.~6]{Deffner_et_al_2022}, the DAG~\ref{fig-cj12_dag} depicts
the conceptual-population outcome \(O^{cp}_{RPUV}\) as an unobserved
variable, emphasizing the researchers' inability to access such an
outcome due to the sampling mechanism. Moreover, the DAG shows the
conditioned \emph{sample design} variable \(S\), which causally
influence the sample-comparison outcome \(O^{sc}_{RPUV}\) . This
variable reflects how this sample-comparison outcome is a sample drawn
from the conceptual-population outcome \(O^{cp}_{RPUV}\). Specifically,
the variable is a vector \(S = \{ s_{1}, \dots, s_{n} \}\), where \(n\)
corresponds to the number of rows in the matrix \(RPUV\) (refer to
