...
are latent vectors of length \(n_{I} \cdot n_{A}\).
...
columns (variables), while \(e_{JK}\) and \(B_{JK}\) are latent vectors
of length \(n_{J} \cdot n_{K}\).
...
population. Moreover, in Section~\ref{sec-theory-theoretical_SC2}, the
...
equation (\ref{eq-mat22}). We apply the same logic to \(S_{J}\), based
on the vector \(J\) (not shown). Thus, the vectors \(S_{I}\) and
\(S_{J}\) contains \(n_{I}\) and \(n_{J}\) elements, respectively.
...
equation (\ref{eq-mat23}). We apply the same logic to \(S_{JK}\), based
on the matrix \(JK\) (not shown). Thus, the vectors \(S_{IA}\) and
...
stimuli produced by the sampled individuals}. This process generates the
...
the subset of judges \emph{does not} perform all repeated judgments for
the subset of stimuli produced by the sampled individuals. Instead, they
