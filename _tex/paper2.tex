...
\subsubsection{The general idea}\label{sec-appendix-B1}
...
outlined in Section~\ref{sec-appendix-B1}. SCM \ref{fig-scm_example1}
and DAG \ref{fig-mdag_example1} depict the plausible causal structure
for this example. In this context, the variable \(X\) (socio-economic
status of the school) acts as a confounder in the relationship between
...
outlined in Section~\ref{sec-appendix-B1}.}
...
Researchers can revisit the scenario in Section~\ref{sec-appendix-B1}
using these insights. By applying the BNF to the SCM
\ref{fig-scm_example1a} and DAG \ref{fig-mdag_example1a} structures,
they can represent the joint probability of the observed variables as
\(P(Y, T, X) =\) \(P(Y \:|\:T, X) P(T \:|\:X) P(X)\). From this
expression, researchers can infer that the outcome \(Y\) is not
unconditionally independent of the teaching method \(T\)
\((Y \:\bot\:T)\). This dependency arises from two key structures: a
...
(black path), and a confounding non-causal path from the teaching method
\(T\) to the outcome \(Y\) through the socio-economic status of the
school \(X\), represented by the fork structure
\(T \leftarrow X \rightarrow Y\) (gray path).
...
school \(X\), achieved by conditioning on \(X\) and depicted by a square
in DAG \ref{fig-mdag_example1a}, effectively blocks all confounding
non-causal path introduced by the fork structure. Nevertheless, since
\(Y\) remains not d-separated from \(T\) even after conditioning
\((Y \:\not\bot\:T \:|\:X)\), this lack of d-separation can only be
...
\subcaption{\label{fig-scm_example1a}SCM}
...
achieve this by relying on the modularity assumption and defining
interventions of the form \(P(Y_{i} \:|\:do(T_{i}=t))\) (see
Section~\ref{sec-appendix-B1}). The \emph{modularity assumption} posits
...
unchanged {[}Pearl\_2010{]}. The probabilistic implications of this
...
causal structure depicted by the SCM \ref{fig-scm_example1a} and DAG
\ref{fig-mdag_example1a} is correct. This means that \(X\) serves as a
...
\ref{fig-mdag_example1a} is equivalent to the manipulated DAG
\ref{fig-mdag_example1b}, because \(X\) satisfies the backdoor
criterion.
...
\subcaption{\label{fig-scm_example1b}SCM}
