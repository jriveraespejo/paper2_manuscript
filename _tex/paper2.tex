...
Moreover, research on its practical applicability highlights CJ's
...
trait measurement and hypothesis testing. This study addresses these
...
and practical elements into a probabilistic statistical model for the
analysis of dichotomous pairwise comparison data.
...
conform to a Normal distribution \citep[pp.~452]{Andrich_1978}. In this
...
\section{Two prominent issues in CJ practice}\label{sec-theory-issues}
...
Table~\ref{tbl-thurstone_cases}). However, a single judge could provide
only \({5 \choose 2} = 10\) unique comparisons, an insufficient data set
...
It is evident that although Thurstone developed Case V to provide a
``rather coarse scaling'' of traits \citep[pp.~269]{Thurstone_1927b} and
cautioned that its use ``should not be made without experimental test''
\citep[pp.~270]{Thurstone_1927b}, the CJ literature has nevertheless
relied the most on it in practice. This preference is particularly
notable given that Case V incorporates the most extensive set of
simplifying assumptions among all the cases
\citetext{\citealp[pp.~253]{Bramley_2008}; \citealp[pp.~677]{Kelly_et_al_2022}}
(see Table~\ref{tbl-thurstone_cases}).
...
guidance on how to use these measurements when, for instance,
researchers aim to test hypotheses related to the measured traits.
Although the CJ practice has circumvented this issue by using scores'
point estimates or their transformations for such analyses, a
fundamental question remains: Is this method appropriate for addressing
these inquiries?

Thus, this section discusses these two prominent issues, as they could
undermine the reliability and validity of CJ's trait estimates
\citep[pp.~2]{Perron_et_al_2015}. Specifically,
Section~\ref{sec-theory-issue1} examines the the excessive reliance on
Thurstone's Case V assumptions in the statistical analysis of CJ data.
Moreover, Section~\ref{sec-theory-issue2} focuses on the apparent
disconnect between the approaches to trait measurement and hypothesis
testing in CJ.

\subsection{The overreliance on Case V in the statistical analysis of CJ
data}\label{sec-theory-issue1}

As previously discussed, despite relying on the most extensive set of
...
assumes a Normal distribution for the stimuli discriminal processes, the
BTL model uses the more mathematically tractable Logistic distribution
\citep[pp.~254]{Andrich_1978, Bramley_2008} (see
Table~\ref{tbl-thurstone_cases}). As noted by
\citet[pp.~16]{vanderLinden_et_al_2017_I}, this substitution has little
...
transformation. Furthermore, this minimal impact is reinforced by the
fact that the Normal and Logistic distributions exhibit analogous
statistical properties, differing only by a scaling factor of
approximately \(1.7\).

However, as mentioned earlier, Thurstone originally developed Case V to
provide a ``rather coarse scaling'' of traits
\citep[pp.~269]{Thurstone_1927b}, prioritizing statistical simplicity
over precision in trait measurement \citep[pp.~677]{Kelly_et_al_2022}.
He explicitly warned against its untested application, stating that its
use ``should not be made without (an) experimental test''
\citep[pp.~270]{Thurstone_1927b}. Furthermore, he acknowledged that some
assumptions could prove problematic when researchers assess complex
traits or heterogeneous stimuli \citep[pp.~376]{Thurstone_1927a}.
...
the BTL model may not consistently hold in theory or practice, namely
the assumption of equal dispersion and zero correlation between stimuli.

\subsubsection{The assumption of equal dispersions between
stimuli}\label{sec-theory-issue1a}

Thurstone argued that the assumption of equal dispersions may not apply
when researchers assess complex traits or heterogeneous stimuli
...
Indeed, the CJ literature may already provide evidence of this violation
through ``misfit'' statistics. For example, labeling texts as
\emph{misfits} indicates that comparisons involving these texts result
in more judgment discrepancies than those involving other texts
\citetext{\citealp[pp.~11]{Pollitt_2004}; \citealp[pp.~290]{Pollitt_2012b}}.
These discrepancies imply that the discriminal differences for
\emph{misfit} texts have broader distributions, further suggesting that
their discriminal processes exhibit more dispersion than those of other
texts. This idea can be illustrated using
Figure~\ref{fig-discriminal_process}, where text B shows a wider
discriminal dispersion than text A. As a result, the texts' discriminal
processes overlap more, making it difficult for a judge to determine
which text possesses more of the trait.

A similar line of reasoning applies to the concept of ``misfit'' judges,
whose evaluations deviate substantially from the shared consensus due to
the unique characteristics of the stimuli or the judges themselves
...
These ``misfit'' judges and their associated deviations can give rise to
additional statistical and measurement issues, which we discuss in more
detail in Section~\ref{sec-theory-issue1b}.
...
experiment can illustrate this idea. In this experiment, researchers can
observe the discriminal processes for the texts depicted in
...
associated with the perception of Text B compared to Text A
...
of the texts are equal \((\sigma_{B} - \sigma_{A}=0)\), the discriminal
difference is more narrow compared to situations where their dispersions
differ. Consequently, the discriminal difference is more likely to favor
Text B over Text A (shaded gray area).
...
Consequently, the ability of the outcome to reflect \emph{true}
differences between stimuli largely depends on the validity of the
model's assumptions \citep[pp.~150]{Kohler_et_al_2019}, particularly the
assumption of equal dispersions. For example, when researchers correctly
assume equal dispersions between texts, the BTL model estimates a
discriminal difference distribution that accurately represents the
\emph{true} discriminal difference between them. This scenario is
...
However, erroneously assuming equal dispersions between stimuli can give
rise to significant statistical and measurement issues. Specifically,
the model may overestimate the trait's reliability, that is, the degree
to which the outcome accurately reflects the \emph{true} discriminal
differences between stimuli. This overestimation, in turn, results in
spurious conclusions about these differences
...
In contrast, Thurstone assumed that stimuli were uncorrelated because
judges' biases, arising from two opposing and equally weighted effects
occurring during the pairwise comparisons, canceled each other out
...
However, the assumption of zero correlation is not met in practice in at
least two scenarios. First, when intricate aspects of multidimensional,
complex traits or heterogeneous stimuli influence judges' perceptions,
generating biases that resist cancellation. Second, when an additional
hierarchical structure is relevant to the stimuli.
...
perceptions, creating biases in their judgments that resist
...
perceptions, these biases could ultimately introduce dependencies
between the stimuli \citep[pp.~346]{vanderLinden_et_al_2017_II}. While
direct evidence for this particular scenario is lacking, studies such as
...
the individual hierarchical level
...
According to the theory, the correlation \(\rho\) measures how much the
...
similar thought experiment, as the one depicted in
...
reverse. They begin by observing the outcomes and use the BTL model to
estimate the stimuli' discriminal difference and discriminal processes.
Given that the BTL model assumes independent discriminal processes
across comparisons, if this assumption holds, the model estimates a
discriminal difference distribution that accurately reflects the
\emph{true} discriminal difference between the texts. This scenario is
illustrated in Figure~\ref{fig-correlation} when the discriminal
difference distribution of the model aligns with the \emph{true}
distribution, represented by the thick continuous line corresponding to
\(\rho=0\). Once more, the accuracy of the discriminal difference
ensures reliable estimates for the discriminal processes of the texts.

However, erroneously assuming zero correlation between stimuli can lead
to significant statistical and measurement issues. Specifically,
neglecting judges' biases or hierarchical structures can create
dimensional mismatches in the model, leading to the over- or
underestimation of trait reliability \citep[pp.~341,
482]{Hoyle_et_al_2023}. In other words, the outcome may fail to
accurately reflect the \emph{true} discriminal differences between
stimuli. These inaccuracies can result in spurious conclusions about
these differences \citep[pp.~370]{McElreath_2020} and, by extension, the
underlying discriminal processes \citep{Ackerman_1989}. This issue is
illustrated in Figure~\ref{fig-correlation} when the model discriminal
difference distribution follows the thick continuous line
\((\rho = 0)\), while the \emph{true} discriminal difference follows any
discontinuous line where \((\rho \neq 0)\). Conversely, excluding judges
based on misfit statistics risks discarding valuable information,
potentially biasing the trait estimates
\citep[chap.~12]{Zimmerman_1994, McElreath_2020}. Again, the direction
and magnitude of these biases are often unpredictable, as they depend on
which judges are excluded from the analysis.
...
limitations, the BTL model commonly serves as the measurement model for
CJ assessments. Specifically, the model provides a method for
transforming manifest into latent variables (psychological scales) and
allocating the compared stimuli on this continuum
\citetext{\citealp[pp.~269]{Thurstone_1927b}; \citealp{Everitt_et_al_2010}}.
For example, when evaluating writing quality, researchers use the BTL
model to process the dichotomous outcomes resulting from the pairwise
comparisons (the manifest variables) to estimate scores that reflect the
underlying level of writing quality (the latent variable)
\citep{Laming_2004, Pollitt_2012b, Whitehouse_2012, vanDaal_et_al_2016, Lesterhuis_2018_thesis, Coertjens_et_al_2017, Goossens_et_al_2018, Bouwer_et_al_2023}.

Researchers then typically use the BTL scores' point estimates, or their
...
In aggregate, excessive reliance on Thurstone's Case V assumptions in
...
coupled with the disconnect between CJ's approach to trait measurement
and hypothesis testing. However, the structural approach to causal
inference can address these issues by offering a systematic and
integrated framework that strengthens measurement reliability and
validity while enhancing the statistical accuracy of hypothesis tests.
...
A motivating example that will appear throughout the document clarifies
these concepts. In this example, researchers aim to determine: ``To what
extent do different teaching methods influence students' ability to
produce high-quality written texts?'' To investigate this, researcher
design a CJ experiment by randomly assigning students (individuals) to
two groups, each receiving a different teaching method. Judges then
compare pairs of students' written texts (stimuli), to produce a
dichotomous outcome that reflects the relative quality of each text
(trait). Based on this setup, researchers reformulate the research
question as the estimand: ``\emph{On average}, is there a difference in
the ability to produce high-quality written texts between the two groups
of students?''. Finally, following current CJ practices, researchers
then rely on estimates from the BTL model, or its transformations, to
approximate this estimand.
...
formalizes the combination of the core theoretical principles outlined
in Section~\ref{sec-thurstone_theory} with key assessment design
features relevant to CJ experiments, such as the selection of judges,
stimuli, and comparisons. In addition to enhancing statistical accuracy
and strengthening measurement reliability and validity, the approach
offers two key advantages. First, it clarifies the interactions among
all actors and processes involved in CJ experiments. Second, it shifts
the current comparative data analysis paradigm from passively accepting
the model assumptions to actively testing whether those assumptions fit
...
model}. This model assumes that researchers have access to a
\emph{census} of comparative data, that is, data representing all
repeated judgments made by every available judge for each pair of
stimuli produced by each pair of individuals in the population, hence
its name. Conversely, Section~\ref{sec-theory-theoretical_SC} integrates
the assessment design features into what we call the
...
\subsubsection{Integrating initial theoretical
...
SCM~\ref{fig-cj03_scm} presents the initial integration of theoretical
principles into the conceptual-population model for the example, applied
to examine the effect of different teaching methods on students' writing
ability. This SCM maps the relationship between the
...
represent the compared texts (stimuli). Moreover, the index \(j\) refers
to the judge, and \(k\) specifies the judgment number, accounting for
experimental conditions where judges perform the same judgment multiple
times, i.e., a \emph{repeated measures design}
\citep[pp.~366-376]{Lawson_2015}. Notably, the indexing system allows
comparisons between texts written by the same student (\(h = i\),
...
as such comparison lacks practical meaning within the CJ framework.
...
Additionally, based on the arguments developed under the first scenario
of Section~\ref{sec-theory-issue1b}, and the recommendations of
...
variables form the preliminary set of endogenous variables,
\(V = \{ O_{hiabjk}, D_{hiabjk}, T_{ha}, T_{ib}, B_{kj} \}\). Finally,
the SCM shows the preliminary set of structural equations,
...
influence, while the absence of an edge implies no relationship between
...
\citep{McElreath_2020}. DAGs conventionally represent observed variables
with solid black circles and unobserved (latent) variables with open
circles \citep{Morgan_et_al_2014}. They also display conditioned
variables by surrounding them with a square. Here, \emph{conditioning}
refers to restricting the statistical analysis to a subset of the
population defined by the values of the given variable
\citep[pp.~32]{Neal_2020}. While DAGs often omit exogenous variables for
simplicity, following a \emph{standard representation}, this section
adopts the \emph{magnified representation} of DAGs, as including
exogenous variables can improve clarity and help reveal potential issues
...
SCM~\ref{fig-cj03_scm}, illustrating the causal relationships among
variables based on Thurstone's theory. Particularly, the graph shows how
the texts' discriminal processes \((T_{ha}, T_{ib})\) influence their
discriminal difference \((D_{hiabjk})\), which in turn determines the
outcome \((O^{cp}_{hiabjk})\). It also depicts the judges' biases
\((B_{kj})\) as an additional factor influencing the texts' discriminal
difference. Furthermore, the DAG distinguish between observed endogenous
variables, such as the outcome, and latent endogenous variables, which
include the texts' discriminal processes, their discriminal difference,
and the judges' biases.

\subsubsection{The comparative census data
structure}\label{sec-theory-theoretical_P2}

Although specifying a system's data structure is not mandatory when
using SCMs and DAGs, defining one can improve clarity and facilitate the
description of the system. Thus, to re-express the scalar form of the CJ
system, outlined in the previous section, into an simpler equivalent
...
where an element of \(I\) corresponds to a unique individual \(h\) or
\(i\), while each row of \(IA\) corresponds to a unique pairing of
individuals \(h, i\) with stimuli \(a, b\). Here, \(n_{I}\) represents
the number of individuals, while \(n_{A}\) denotes the number of stimuli
available per individual. Thus, vector \(I\) has a length of \(n_{I}\),
and matrix \(IA\) has a dimension of \((n_{I} \cdot n_{A})\) rows and
\(2\) columns. The vector and the matrix consist of scalars from the set
of natural numbers \(\mathbb{N}\).

Similarly, an element of the vector \(J\) corresponds to a unique judge
\(j\). Moreover, each row of \(JK\) specifies a unique pairing of a
judge \(j\) with a judgment number \(k\). In this case, \(n_{J}\)
indicates the total number of judges, and \(n_{K}\) represents the
number of judgments each judge performs. As a result, vector \(J\) has a
length of \(n_{J}\), and matrix \(JK\) has a dimension of
\((n_{J} \cdot n_{K})\) rows and \(2\) columns. Like the previous
structures, the vector and the matrix consist of scalars from the set of
natural numbers \(\mathbb{N}\).

Additionally, we define the matrix \(RPUV\), where each row uniquely
pairs the corresponding rows from the matrices \(IA\) and \(JK\). This
matrix reflects the fundamental idea that the discriminal difference of
the stimuli becomes an observable outcome only through the judges'
perceptions. Thus, the matrix is defined as follows:

\begin{equation}\phantomsection\label{eq-mat3}{
...
This matrix also consists of scalars from the set of natural numbers
\(\mathbb{N}\) and has a dimension of
\(n = {n_{I} \cdot n_{A} \choose 2} \cdot n_{J} \cdot n_{K}\) rows and
\(6\) columns. The matrix's dimension results from the
conceptual-population model's assumption that researchers have access to
a \emph{census} of comparative data, capturing all repeated judgments
made by every available judge for each pair of stimuli produced by each
pair of individuals in the population. Here, the term
\({n_{I} \cdot n_{A} \choose 2}\) represents the Binomial coefficient,
which quantifies the total number of individual-stimuli comparison pairs
within the conceptual population. The coefficient is defined as follows:

\[
{n_{I} \cdot n_{A} \choose 2} = \frac{ (n_{I} \cdot n_{A})! }{ 2! (n_{I} \cdot n_{A} - 2)! }
\]

We visualize the structure of these vectors and matrices by considering
small values for the parameters corresponding to number of individuals,
the number of available stimuli per individual, the total number of
judges, and the number of judgments each judge performs. For instance,
assuming \(n_{I} = 5\), \(n_{A} = 2\), \(n_{J} = 3\), and \(n_{K} = 3\),
the matrices described in equations (\ref{eq-mat12}) and (\ref{eq-mat3})
...
Thus, using the data structure defined by the matrices in equations
(\ref{eq-mat12}) and (\ref{eq-mat3}), we can re-express
SCM~\ref{fig-cj03_scm} and DAG~\ref{fig-cj03_dag} in a simpler
equivalent vectorized form, as shown in Figure~\ref{fig-cj04}. In this
depiction, the outcome \(O^{cp}_{RPUV}\), the texts' discriminal
difference \(D_{RPUV}\), their discriminal processes \(T_{IA}\), and the
judges' biases \(B_{JK}\) are represented as vectors rather than scalar
values. These vectors encompass all values from the conceptual
population. Specifically, \(O^{cp}_{RPUV}\) is an observed vector of
length \({n{I} \cdot n_{A} \choose 2} \cdot n_{J} \cdot n_{K}\), while
\(D_{RPUV}\) is a latent vector of the same dimension. Additionally,
\(T_{IA}\) and \(B_{JK}\) are latent vectors of lengths
\((n_{I} \cdot n_{A})\) and \((n_{J} \cdot n_{K})\), respectively.
...
the conceptual-population model incorporates two \emph{hierarchical
structural components}. Each component describes how different observed
or latent variables influence the primary latent variable of interest
\citep{Everitt_et_al_2010}. This addition addresses the problems arising
when an additional hierarchical structure is relevant to the stimuli, as
outlined in the second scenario of Section~\ref{sec-theory-issue1b}.
Furthermore, it facilitates analyses that integrate both trait
measurement and hypothesis testing processes, thereby accounting for
both traits and their associated uncertainties, reducing bias and
improving measurement precision (see Section~\ref{sec-theory-issue2}).

Figure~\ref{fig-cj09} displays the two hierarchical structural
components for the example, applied to examine the effect of different
teaching methods on students' writing ability. Specifically, the top
branch of DAG~\ref{fig-cj09_dag} depicts the first component. This
component describes how the students' writing-quality trait \((T_{I})\)
and a set of \emph{relevant} text-related variables \((X_{IA})\), such
as text length, causally influence the texts' written-quality trait
\((T_{IA})\). The DAG also highlights the idiosyncratic errors
\((e_{IA})\), which capture variations in the texts' trait that neither
the students' trait nor other relevant text-related variables can
explain. Here, \(T_{I}\) and \(e_{IA}\) are latent vectors of length
\(n_{I}\) and \((n_{I} \cdot n_{A})\), respectively. Additionally,
\(X_{IA}\) is an observed matrix with a dimension of
\((n_{I} \cdot n_{A})\) rows and \(q_{IA}\) columns.

Furthermore, the top branch of DAG~\ref{fig-cj09_dag} describes how a
set of \emph{relevant} student-related variables \((X_{I})\), including
teaching methods, causally influence the students' writing-quality trait
...
vector of length \(n_{I}\), and \(X_{I}\) is an observed matrix with
\(n_{I}\) rows and \(q_{I}\) columns.
...
\((Z_{JK})\), such as number of judgments or time for judgment, causally
...
Moreover, the bottom branch of DAG~\ref{fig-cj09_dag} describes how a
set of \emph{relevant} judge-related variables \((Z_{J})\), including
judgment expertise, causally influence the judges' bias \((B_{J})\). The
DAG also accounts for idiosyncratic errors \((e_{J})\), capturing the
variations in the judges' bias that relevant judge-related variables
cannot explain. Here, \(e_{J}\) is a latent vector of length \(n_{J}\),
and \(Z_{J}\) is an observed matrix with \(n_{J}\) rows and \(q_{J}\)
columns.
...
in Section~\ref{sec-theory-issues}. These enhancements include
accounting for judges' biases and integrating hierarchical structural
components. Nevertheless, despite its promise of enhancing measurement
accuracy and precision, the model still depends on the unrealistic
assumption that researchers have access to a \emph{census} of
comparative data, encompassing all repeated judgments from every judge
for each pair of stimuli produced by every individual in the population.
Since researchers rarely meet this assumption in practice, they must
consider a more realistic scenario. The extensions to the
conceptual-population model, derived from this realistic scenario, are
...
In the sample-comparison model, we assume a more realistic scenario
compared to the conceptual-population model. Specifically, we assume
that researchers have access to \emph{all comparisons} from a limited
number of judgments \((n^{s}_{K})\) from a sample of judges
\((n^{s}_{J})\), along with a specific number of texts \((n^{s}_{A})\)
from a sample of students \((n^{s}_{I})\) drawn from the conceptual
population. Figure~\ref{fig-cj12} illustrates this model, incorporating
only this sample mechanism.

Following the convention of \citet[pp.~499-516]{McElreath_2020}, the
DAG~\ref{fig-cj12_dag} depicts the conceptual-population outcome
\(O^{cp}_{RPUV}\) as an unobserved variable, emphasizing the
researchers' inability to access such an outcome due to the sampling
mechanism. Moreover, the DAG shows the conditioned \emph{sample design}
variable \(S\), which causally influence the sample-comparison outcome
\(O^{sc}_{RPUV}\). This variable reflects how this sample-comparison
outcome is a sample drawn from the conceptual-population outcome
\(O^{cp}_{RPUV}\). Specifically, the variable is a vector
\(S = \{ s_{1}, \dots, s_{n} \}\), where \(n\) corresponds to the number
of rows in the matrix \(RPUV\) (refer to
Section~\ref{sec-theory-theoretical_P2}), and each \(s_{i}\) is a binary
variable indicating the presence \((1)\) or absence \((0)\) of data in
the corresponding positions of the conceptual-population outcome
\(O^{cp}_{RPUV}\). Specifically,
...
\subcaption{\label{fig-cj12_scm}SCM}
...
\includegraphics[width=0.84\linewidth,height=\textheight,keepaspectratio]{./images/png/CJ_TM_12.png}

}

\subcaption{\label{fig-cj12_dag}DAG}
...
\(O^{sc}_{RPUV}\) as an unobserved variable, emphasizing the
researchers' inability to access such an outcome due to the comparison
mechanism. Moreover, the DAG shows the conditioned \emph{comparison
design} variable \(C\), which causally influence the observed outcome
\(O_{RPUV}\). This variable reflects how this observed outcome is a
sample drawn from the sample-comparison outcome \(O^{sp}_{RPUV}\).
Specifically, the variable is a vector
\(C = \{ c_{1}, \dots, c_{n} \}\), where \(n\) corresponds to the number
of rows in the matrix \(RPUV\) (refer to
...
\subcaption{\label{fig-cj15_stat}Statistical model}
...
comparison mechanisms and assuming equal discriminal dispersions for the
student's traits}
