...
in detail. Section~\ref{sec-theoretical} develops Thurstone's theory to
address these challenges. The development integrates core theoretical
...
\section{Developing Thurstone's theory}\label{sec-theoretical}
...
researcher design a CJ experiment by randomly assigning students
...
text (trait). Based on this setup, researchers reformulate the research
...
Section~\ref{sec-theory-issues} by developing Thurstone's theory using
...
In line with Thurstone's theory, the other variables present in
SCM~\ref{fig-cj03_scm} include the texts' discriminal processes
\((T_{ha}, T_{ib})\) and their discriminal difference \((D_{hiabjk})\)
(see Section~\ref{sec-thurstone_theory}). Additionally, based on the
arguments presented in Section~\ref{sec-theory-issue1b} and the
recommendations of \citet{Andrich_1978} and \citet{Wainer_et_al_1978},
the SCM~\ref{fig-cj03_scm} incorporates the judges' biases \((B_{kj})\).
Together with the outcome, these variables form the preliminary set of
...
\caption{\label{fig-cj03}Conceptual-population model, preliminary
...
extend from one node to another, showing the direction of causal
influence. The term \emph{acyclic} means that the causal influences do
...
\citep{Morgan_et_al_2014}. Moreover, they often omit exogenous variables
for simplicity, following the \emph{standard representation}. However,
this section adopts the \emph{magnified representation} of DAGS, as
including exogenous variables can enhance clarity and reveal potential
issues related to conditioning and confounding
\citep{Cinelli_et_al_2020}.

Figure~\ref{fig-cj03_dag} displays the DAG associated with
SCM~\ref{fig-cj03_scm}. This graph illustrates the direction of causal
influences among variables, reflecting the assumptions of Thurstone's
theory and the practical aspects of CJ experiments. It also
distinguishes between observable endogenous variables, such as the
outcome, and variables considered latent within the CJ framework, such
as the text's discriminal processes, their discriminal difference, and
the judges' biases.
...
the description of the system. Thus, to simplify the description of the
CJ system, we first define the vectors \(I\) and \(J\), along with the
...
of individuals \((n_{I} = 5)\), the number of available stimuli per
individual \((n_{A} = 2)\), the total number of judges \((n_{J} = 3)\),
and the number of judgments each judge performs \((n_{K} = 3)\). Using
these values, the matrices described in equations (\ref{eq-mat12}) and
...
(\ref{eq-mat12}) and (\ref{eq-mat3}), we can now re-express
SCM~\ref{fig-cj03_scm} and DAG~\ref{fig-cj03_dag} in an equivalent
vectorized form, as shown in Figure~\ref{fig-cj04}. In this
representation, the outcome \(O^{cp}_{RPUV}\), the texts' discriminal
difference \(D_{RPUV}\), their discriminal processes \(T_{IA}\), and the
judges' biases \(B_{JK}\) are no longer scalar values but vectors. These
vectors capture all the values from the conceptual population.
Specifically, \(O^{cp}_{RPUV}\) and \(D_{RPUV}\) are vectors of length
\({n_{I} \cdot n_{A} \choose 2} \cdot n_{J} \cdot n_{K}\), while
\(T_{IA}\) and \(B_{JK}\) are vectors of length \((n_{I} \cdot n_{A})\)
and \((n_{J} \cdot n_{K})\), respectively.
...
\caption{\label{fig-cj04}Conceptual-population model, vectorized form of
SCM~\ref{fig-cj03_scm} and DAG~\ref{fig-cj03_dag}.}

\end{figure}%

Building on the arguments developed in Section~\ref{sec-theory-issue2}
and the principles of Structural Equation Modeling (SEM)
...
\citep{Everitt_et_al_2010}. This addition facilitates analyses that
integrate the trait measurement and hypothesis testing processes,
simultaneously accounting for the traits and their associated
uncertainties, thereby reducing bias and enhancing the measurement
precision.
...
describes how the students' writing-quality trait \(T_{I}\) and a set of
\emph{relevant} text-related variables \(X_{IA}\) causally influence the
texts' written-quality trait \(T_{IA}\). The DAG also highlights the
idiosyncratic errors \(e_{IA}\), which capture variations in the texts'
trait that neither the students' trait nor other relevant text-related
variables can explain. Here, \(T_{I}\) and \(e_{IA}\) are latent vectors
of length \(n_{I}\) and \((n_{I} \cdot n_{A})\), respectively.
Additionally, \(X_{IA}\) is an observed matrix with a dimension of
\((n_{I} \cdot n_{A})\) rows and \(p_{IA}\) columns.
...
includes the idiosyncratic errors \(e_{I}\), which capture variations in
the the students' traits that the relevant student-related variables
cannot explain. Here, \(e_{I}\) is a latent vector of length \(n_{I}\),
and \(X_{I}\) is an observed matrix with \(n_{I}\) rows and \(p_{I}\)
columns.
...
\caption{\label{fig-cj09}Conceptual-population model, full integration
...
\(B_{JK}\). The DAG also highlights the idiosyncratic errors \(e_{JK}\),
which capture variations in the texts' bias that neither the judges'
bias nor other relevant judgment-related variables can explain. In this
description, \(B_{J}\) and \(e_{JK}\) are latent vectors of length
\(n_{J}\) and \((n_{J} \cdot n_{K})\), respectively. Additionally,
\(Z_{JK}\) is an observed matrix with a dimension of
\((n_{J} \cdot n_{K})\) rows and \(p_{JK}\) columns.
...
idiosyncratic errors \(e_{J}\), capturing the variations in the judges'
bias that relevant judge-related variables cannot explain. Here,
\(e_{J}\) is a latent vector of length \(n_{J}\), and \(Z_{J}\) is an
observed matrix with \(n_{J}\) rows and \(p_{J}\) columns. Notably, all
variables and functions shown in SCM~\ref{fig-cj09_scm} and
DAG~\ref{fig-cj09_dag} constitute the set of endogenous variables \(V\)
and structural equations \(F\) for the conceptual-population model.
...
A cautious reader will notice that the model descriptions of
Section~\ref{sec-theory-theoretical_P} rely on several relevant
variables. In this study, a \emph{relevant} or a \emph{sufficient
adjustment} set of variables (potentially an empty set) refers to those
that satisfy the \emph{backdoor criterion} \citep[pp.~37]{Neal_2020},
that is, variables that block all non-causal paths without introducing
new ones \citep{Pearl_2009, Pearl_et_al_2016, Morgan_et_al_2014}.

Incorporating these variables into the conceptual-population model
enables researchers to identify different estimands of interest and
their causal effects, even with observational data. This identification
works because conditioning on these variables ensures that the estimator
meets several critical properties, including confounding elimination
\citep{Morgan_et_al_2014}. Here, \emph{conditioning} means restricting
the statistical analysis to a subset of the population defined by the
values of a given variable \citep[pp.~32]{Neal_2020}.

Naturally, the validity of causal claims now hinges on the assumption
that these variables serve as a sufficient adjustment set, a property
that cannot be guaranteed for any variable. Nevertheless, as
\citet[pp.~150]{Kohler_et_al_2019} noted, drawing conclusions about the
real world from observed data inevitably requires assumptions, and these
requirements hold true for both observational and experimental data.
