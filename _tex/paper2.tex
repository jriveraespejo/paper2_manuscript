...
For instance, a motivating example that illustrates these concepts
throughout the document could involve researchers aiming to answer the
question: ``To what extent do different teaching methods influence
students' ability to produce high-quality written texts?'' To
investigate this, they could design a CJ experiment by randomly
assigning students to two groups, each receiving a different teaching
method. Judges would then compare pairs of students' written texts,
producing a dichotomous outcome that reflects the relative quality of
each text. Based on this setup, researchers can reformulate the research
...
These limitations hinder the model's ability to identify distinct
...
researchers cannot achieve \emph{consistency} even with ``infinite'' and
error-free data. In other words, the estimates derived from the
estimator would fail to converge to the ``true'' value of the estimand
as the data size approaches infinity \citep{Everitt_et_al_2010}. As a
result, deriving meaningful insights from finite data also becomes
unattainable \citep[pp.~5]{Schuessler_et_al_2023}.
...
the selection of judges, stimuli, and comparisons. The extended model
clarifies the interactions among all actors and processes involved in CJ
experiments. Furthermore, it allows researchers to shift the current
comparative data analysis paradigm from accepting the model's
assumptions as given to testing these assumptions for specific
comparative data. For details on these assumptions, refer to
Section~\ref{sec-theory-issues}. Accordingly,
Section~\ref{sec-theory-theoretical_P} incorporates the theoretical
principles into what the study calls the \emph{conceptual-population
model}. Meanwhile, Section~\ref{sec-theory-theoretical_SC} integrates
the assessment design features into what the study terms the
\emph{sample-comparison model}.
...
Section~\ref{sec-thurstone_theory}, the study defines SCMs and DAGs.
SCMs are formal mathematical models characterized by a set of
...
endogenous variables as non-parametric functions of other variables.
...
Figure~\ref{fig-cj04} illustrates the preliminary integration of the
theoretical principles underlying the motivating example. Specifically,
SCM~\ref{fig-cj04_scm} displays the relationship between the outcome of
the CJ experiment, \(O_{abhijk}\), to several other variables. Here, the
indices \(\{a,b\}\) represent the compared texts, \(\{h,i\}\) label the
students who wrote these texts, \(j\) denotes the judge, and \(k\)
accounts for cases where the judges perform multiple comparisons of the
same texts written by the same students, i.e., the comparison number for
a \emph{repeated measures design} \citep[pp.~366-376]{Lawson_2015}. The
other variables related to the outcome include the discriminal
difference \((D_{abhijk})\), the perceived discriminal processes
\((PT_{ahjk}, PT_{bijk})\), the ``true'' discriminal processes
\((T_{ahjk}, T_{bijk})\), and the judges' biases \((B_{kj})\). Together
with the outcome, these variables form the preliminary set of endogenous
variables
\(V = \{O_{abhijk}, D_{abhijk}, PT_{ahjk}, PT_{bijk}, T_{ahjk}, T_{bijk}, B_{kj}\}\).
Finally, the preliminary set of structural equations
\(F = \{f_{O},f_{D},f_{PT}\}\) defines the dependencies among these
variables.
...
DAG~\ref{fig-cj04_dag} displays the preliminary DAG associated with
...
theory. It also distinguishes between observable endogenous variables,
such as the outcome \(O_{abhijk}\), and variables considered latent
according to the theory and practice of CJ, such as the discriminal
difference \((D_{abhijk})\), the perceived discriminal processes
\((PT_{ahjk}, PT_{bijk})\), the ``true'' discriminal processes
\((T_{ahjk}, T_{bijk})\), and the judges' biases \((B_{kj})\).
