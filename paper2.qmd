...
<!-- 2. assumptions about the outcome, and the sample and comparison mechanism -->
...
<!-- 2. Continuing with the assumptions -->
...
Finally, using *Bayesian inference methods* to transform the [model -@fig-cj16_stat] into a practical statistical tool for analyzing paired comparison data. Bayesian inference offers three key advantages. First, it is well-suited for estimating complex and overparameterized models, where the number of parameters may exceed the number of observations [@Baker_1998; @Kim_et_al_1999]. Second, it allows researchers to incorporate prior information, which constrains parameter estimates within plausible bounds. This feature mitigates common estimation issues, such as non-convergence or improper solutions, often encountered in complex models under frequentist methods [@Martin_et_al_1975; @Seaman_et_al_2011]. Third, Bayesian inference is particularly effective for drawing conclusions from small samples, where the asymptotic properties underlying frequentist methods are less reliable [@Baldwin_et_al_2013; @Lambert_et_al_2006; @Depaoli_2014]. The **Declarations** section of this document provides a link to the model code, along with an alternative specification that assumes equal discriminal dispersions. We implemented both versions of the model using `Stan` [@Stan_2020, version 2.26.1]. Although the implementations were successful, we do not report the results here, as they fall outside the scope of this document.
