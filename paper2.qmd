...
Even though counterfactuals are unobserved and unobservable, researchers can still identify the ACE from associational estimates by leveraging the structural approach. The approach identifies the ACE by statistically conditioning data on a *sufficient adjustment set* of variables $X$ [@Pearl_2009; @Pearl_et_al_2016; @Morgan_et_al_2014]. This *sufficient* set (potentially empty) must block all non-causal paths between $T$ to $Y$ without opening new ones. If such a set exists, then $T$ and $Y$ are *d-separated* by $X$ ($T \dsep Y \cond X$) [@Pearl_2009], and $X$ satisfies the *backdoor criterion* [@Neal_2020, pp 37]. Here, *conditioning* describes the process of restricting the focus to the subset of the population defined by the conditioning variable [@Neal_2020, pp. 32] (see @eq-group_diff). 
...
The structural approach to causal inference translate the probabilistic insights from @sec-appendix-B23 into actionable strategies for identifying the ACE from associational quantities. The approach achieve this by relying on the modularity assumption and by re-defining interventions in the form $P(Y_{i} \cond do(T_{i}=t))$ (see @sec-appendix-B). The *modularity assumption* posits that intervening on a node alters only the causal mechanism of that node, leaving others unchanged [@Neal_2020, pp. 34].
