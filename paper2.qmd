...
<!-- 2. The DAGs -->
Notably, every SCM has an associated DAG [@Pearl_et_al_2016; @Cinelli_et_al_2020]. A DAG is a *graph* consisting of nodes connected by edges, where nodes represent random variables. The term *directed* indicates that edges or arrows extend from one node to another, indicating the direction of causal influence. The absence of an edge implies no direct relationship between the nodes. The term *acyclic* means that the causal influences do not form loops, ensuring the influences do not cycle back on themselves [@McElreath_2020]. DAGs conventionally depict observed variables as solid black circles and unobserved (latent) variables as open circles [@Morgan_et_al_2014]. They also display conditioned variables by surrounding them with a square. Here, *conditioning* refers to restricting the statistical analysis to a subset of the population defined by the values of the given variable [@Neal_2020, pp. 32]. Although DAGs often omit exogenous variables for simplicity, this section includes exogenous variables to improve clarity and reveal potential issues related to conditioning and confounding [@Cinelli_et_al_2020].
...
<!-- 2. the preliminary DAG for CJ -->
Moreover, @fig-cj03_dag displays the DAG corresponding to [SCM -@fig-cj03_scm], illustrating the expected causal outlined in Thurstone's theory. The graph shows that the discriminal processes of the texts $(T_{ha}, T_{ib})$ influence their discriminal difference $(D_{hiabjk})$, which in turn determines the outcome $(O^{cp}_{hiabjk})$. It also highlights the influence of judges' biases $(B_{kj})$ on the discriminal difference. Additionally, the DAG differentiates between observed endogenous variables, such as the outcome, and latent endogenous variables, including the texts' discriminal processes, their discriminal difference, and the judges' biases.
...
Here, each element of $I$ represents a unique individual $h$ or $i$, where $n_{I}$ denotes the total number of individuals. Similarly, each element of $J$ corresponds to a unique judge $j$, with $n_{J}$ indicating the total number of judges. Moreover, each row of $IA$ represents a unique pairing of individuals $h, i$ with stimuli $a, b$. As a result, the matrix $IA$ contains $(n_{I} \cdot n_{A})$ rows and $2$ columns, where $n_{A}$ specifies the number of stimuli available per individual. Likewise, each row of $JK$ associates a judge $j$ with a judgment index $k$, where $n_{K}$ indicates the number of judgments each judge makes. Consequently, the matrix $JK$ has $(n_{J} \cdot n_{K})$ rows and $2$ columns. 

Additionally, we define the matrix $RPUV$ that assigns each row from the $IA$ matrix to a corresponding row from the $JK$ matrix. Thus, the matrix consists of $n = {n_{I} \cdot n_{A} \choose 2} \cdot n_{J} \cdot n_{K}$ rows and $6$ columns. Here, the term ${n_{I} \cdot n_{A} \choose 2}$ represents the Binomial coefficient, which quantifies the total number of repeated judgments each judge makes for every pair of stimuli generated by each pair of individuals in the population. The matrix is defined as follows:
...
It is easier to visualize the structure of these vectors and matrices by considering an example where $n_{I} = 5$, $n_{A} = 2$, $n_{J} = 3$, and $n_{K} = 3$. In this simple case, the vectors and matrices described in equations ([-@eq-mat12]) and ([-@eq-mat3]) take the following form:
...
Then, using equations ([-@eq-mat12]) and ([-@eq-mat3]), we can re-express [SCM -@fig-cj03_scm] and [DAG -@fig-cj03_dag] in an equivalent vectorized form, as shown in @fig-cj04. In this depiction, the outcome $O^{cp}_{RPUV}$, the texts' discriminal difference $D_{RPUV}$, their discriminal processes $T_{IA}$, and the judges' biases $B_{JK}$ are represented as vectors rather than scalar values. These vectors capture all the observations from the conceptual population. Specifically, $O^{cp}_{RPUV}$ is an observed vector of length $n={n_{I} \cdot n_{A} \choose 2} \cdot n_{J} \cdot n_{K}$, and $D_{RPUV}$ is a latent vector of the same dimension. Moreover, $T_{IA}$ and $B_{JK}$ are latent vectors of lengths $(n_{I} \cdot n_{A})$ and $(n_{J} \cdot n_{K})$, respectively.
