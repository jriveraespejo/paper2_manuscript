...
In the second scenario, the shared context or inherent connections created by additional hierarchical structures may further introduce dependencies between stimuli, creating a statistical phenomenon known as clustering [@Everitt_et_al_2010]. Although the CJ literature acknowledges the existence of such hierarchical structures, the statistical handling of this additional source of dependence between stimuli has been inadequate. For instance, when CJ data incorporates multiple samples of stimuli from the same individuals, researchers frequently rely on (average) estimated BTL scores to conduct subsequent analyses and tests at the individual hierarchical level [@Bramley_et_al_2019; @Boonen_et_al_2020; @Bouwer_et_al_2023; @vanDaal_et_al_2017; @Jones_et_al_2019; @Gijsen_et_al_2021]. This approach, however, can introduce additional statistical and measurement issues, which we discuss in greater detail in @sec-theory-issue2.
...
In aggregate, the excessive reliance on Thurstone's Case V assumptions in the statistical analysis of CJ data can compromise the reliability of CJ's trait estimates. This overreliance, coupled with the apparent disconnect between CJ's approach to trait measurement and hypothesis testing, may also undermine the validity of these estimates [@Perron_et_al_2015, pp. 2]. Thus, addressing these issues through a more systematic and integrated approach could provide significant statistical and measurement benefits. For this reason, the study employs the structural approach to causal inference, as it fulfills these requirements.
...
# Extending Thurstone's model {#sec-theoretical}
...
A motivating example helps to clarify these concepts. The example will appear throughout the document to illustrate various aspects of the proposed extension. In this scenario, researchers aim to determine: "To what extent do different teaching methods influence students' ability to produce high-quality written texts?" To investigate this, they could design a CJ experiment by randomly assigning students (individuals) to two groups, each receiving a different teaching method. Judges would then compare pairs of students' written texts (stimuli), producing a dichotomous outcome that reflects the relative quality of each text (trait). Based on this setup, researchers could reformulate the research question as the estimand: "*On average*, is there a difference in the ability to produce high-quality written texts between the two groups of students?". Following current CJ practices, researchers would then rely on estimates from the BTL model, or its transformations, to approximate this estimand.

<!-- 4. But why identification is so important? -->
However, @sec-theory-issues presents compelling evidence that the BTL model has several statistical and measurement limitations. These limitations hinder the model's ability to identify various estimands relevant to CJ inquiries, including the one from the motivating example. Identification is crucial because it is a necessary condition for ensuring consistent estimators. Without identification, *consistency* --the property of an estimator whose estimates converge to the "true" value of the estimand as the data size approaches infinity [@Everitt_et_al_2010]-- cannot be achieved, even with "infinite" and error-free data. As a result, deriving meaningful insights from finite data also becomes unattainable [@Schuessler_et_al_2023, pp. 5].
...
<!-- 5. What the section will do? -->
As a result, this section addresses the identified limitations by extending Thurstone's general form (see @tbl-thurstone_cases) through a systematic and integrated approach based on the structural approach to causal inference. Specifically, it leverages the capabilities of this approach to formalize the combination of the core theoretical principles outlined in @sec-thurstone_theory with key assessment design features relevant to CJ experiments, such as the selection of judges, stimuli, and comparisons. 

<!-- 6. What are the advantages? -->
In addition to addressing the identified limitations, the extended model offers two key advantages. First, it clarifies the interactions among all actors and processes involved in CJ experiments. Second, it shifts the current data analysis paradigm from passively accepting CJ's model assumptions to actively testing them with the specific data under analysis. For more details on these assumptions, refer to @sec-theory-issues.

<!-- 7. How the section is divided? -->
Accordingly, @sec-theory-theoretical_P incorporates the theoretical principles into what the study calls the *conceptual-population model*. This model assumes that researchers have access to a conceptual population of comparison data, meaning data representing all repeated comparisons made by every available judge for each pair of stimuli produced by each pair of individuals in the population, thus its name. Conversely, @sec-theory-theoretical_SC integrates the assessment design features into what the study calls the *sample-comparison model*. This model assumes a more realistic scenario where researchers only have access to a sample of judges, individuals, stimuli, and comparisons from the conceptual population.
...
<!-- 1. The SCMs -->
Before incorporating the theoretical principles into the *conceptual-population model*, the study defines SCMs, which are formal mathematical models characterized by a set of *endogenous* variables $V$, a set of *exogenous* variables $E$, and a set of functions $F$ [@Pearl_2009; @Cinelli_et_al_2020]. Endogenous variables are those whose causal mechanisms a researcher chooses to model [@Neal_2020]. In contrast, exogenous variables represent *errors* or *disturbances* arising from omitted factors that the investigator chooses not to model explicitly [@Pearl_2009, pp. 27,68]. Lastly, the functions, referred to as *structural equations*, express the endogenous variables as non-parametric functions of other endogenous and exogenous variables. These functions use the symbol '$:=$' to denote the asymmetrical causal dependence between variables and the symbol '$\dsep$' to represent *d-separation*, a concept akin to (conditional) independence.

<!-- 2. the preliminary SCM for CJ -->
@fig-cj04 illustrates the preliminary integration of theoretical principles into the conceptual-population model. Specifically, @fig-cj04_scm shows the SCM that underlies the motivating example. This SCM maps the relationship between the outcome of the CJ experiment $(O_{abhijk})$ to several other variables. In this model, the indices $\{a,b\}$ represent the compared texts (stimuli), with $a \neq b$. The indices $\{h,i\}$ label the students who wrote the texts (individuals), where $h = i$ is possible. The index $j$ denotes the judge, and $k$ denotes the comparison number, which accounts for experimental cases where judges perform multiple comparisons of the same texts written by the same students, i.e., a *repeated measures design* [@Lawson_2015, pp. 366-376]. Finally, the SCM presents the preliminary set of structural equations $F = \{f_{O},f_{D},f_{PT}\}$, which define the non-parametric dependencies among these variables.

<!-- 3. What are the innovations of this? -->
Note that the [SCM -@fig-cj04_scm] incorporates not only the variables described by Thurstone's theory, such as the discriminal difference $(D_{abhijk})$ and the texts' *true* discriminal processes $(T_{ahjk}, T_{bijk})$ (refer to @sec-thurstone_theory), but also innovates by incorporating additional variables central to CJ experiments, such as the texts' *perceived* discriminal processes $(PT_{ahjk}, PT_{bijk})$ and the judges' perceptions $(B_{kj})$. Together with the outcome, these variables constitute the preliminary set of endogenous variables $V = \{O_{abhijk}, D_{abhijk}, PT_{ahjk}, PT_{bijk}, T_{ahjk}, T_{bijk}, B_{kj}\}$. 

By incorporating the texts' perceived discriminal processes into the conceptual-population model, this study considers a crucial aspect of CJ experiments that previous literature has overlooked. It recognizes that the texts' true discriminal processes become an observable outcome only through the judges' perceptions. In other words, researchers observe the CJ outcome only when both variables create a *pure interaction effect*, meaning that neither variable has a direct effect on the outcome, but their interaction produces an effect [@Attia_et_al_2022].

Conversely, based on the evidence presented in @sec-theory-issue1b, the study assumes that other relevant stimulus characteristics influence the judges' perceptions, leading to biases that resist cancellation and ultimately introduce dependencies between the stimuli. To address this, the study incorporates the judges' perceptions into the conceptual-population model, following the recommendations of @Andrich_1978 and @Wainer_et_al_1978.


<!-- ![](/images/png/CJ_TM_01.png){#fig-cj01_dag fig-align="center" fig-pos="H" width=23%} -->

<!-- ![](/images/png/CJ_TM_02.png){#fig-cj02_dag fig-align="center" fig-pos="H" width=32%} -->

<!-- ![](/images/png/CJ_TM_03.png){#fig-cj03_dag fig-align="center" fig-pos="H" width=45%} -->
...
$$
\begin{aligned}
  O_{abhijk} & := f_{O}(D_{abhijk}) \\
  D_{abhijk} & := f_{D}(PT_{ahjk}, PT_{bijk}) \\
  PT_{ahjk} & := f_{PT}(T_{ah}, B_{jk}) \\
  PT_{bijk} & := f_{PT}(T_{bi}, B_{jk})
\end{aligned}
$$
...
![DAG](/images/png/CJ_TM_04.png){#fig-cj04_dag fig-align="center" fig-pos="H" width=45%}

CJ's outcome, discriminal difference, discriminal processes and judges' biases
:::

<!-- 4. The DAGs -->
Notably, every SCM has an associated DAG [@Pearl_et_al_2016; @Cinelli_et_al_2020]. A DAG is a graph consisting of nodes connected by edges, where the nodes represent random variables. The term *directed* means that the edges extend from one node to another, with arrows indicating the direction of causal influence. The term *acyclic* implies that the causal influences do not form loops, ensuring the influences do not cycle back on themselves [@McElreath_2020]. DAGs represent observed variables as solid black circles, while they use open circles for unobserved (latent) variables [@Morgan_et_al_2014]. While the *standard representation* of DAGs typically omits exogenous variables for simplicity, the *magnified representation* includes them, offering the advantage of highlighting potential issues related to conditioning and confounding [@Cinelli_et_al_2020]. Thus, this study uses the magnified representation.

<!-- 5. the preliminary DAG for CJ -->
@fig-cj04_dag displays the DAG associated with [SCM -@fig-cj04_scm]. The graph shows the direction of causal influences among variables, reflecting the assumptions of Thurstone's theory and CJ experiments. It also distinguishes between observable endogenous variables, such as the outcome $O_{abhijk}$, and variables considered latent according to the theory and practice of CJ, such as the discriminal difference, the perceived discriminal processes, the true discriminal processes, and the judges' perceptions.

<!-- 6. The problem of continuing this way -->
It is easy to intuit that describing the CJ system by assigning an index and a variable to each discriminal process could quickly become intractable and cumbersome due to the large size of the DAG and the repetition of its components. Therefore, to maintain generality, the study groups pairs of discriminal processes with different indices into matrices that contain these pairs. First, the study defines the vectors $U_{A} \in \mathbb{N}^{A}$, $U_{I} \in \mathbb{N}^{I}$ containing the indices of the stimuli and individuals, where: 

$$
U_{A} = [1, \dots, a, \dots, b, \dots, A] ;\quad U_{I} = [1, \dots, a, \dots, b, \dots, I]
$$ 

Then, the study defines the matrices of stimuli and individual comparisons, $P \in \mathbb{N}^{m \times 2}$ and $R \in \mathbb{N}^{n \times 2}$, respectively. Here, each row of $P$ represents a pairing $\{a,b\}$ of stimuli indices and each row of $R$ represents a pairing $\{h,i\}$ of individual indices, i.e.,

$$
P = \{(a, b) \mid a,b \in U_{A}, a \neq b\} ;\quad R = \{(h, i) \mid h,i \in U_{I}\}
$$

Here, $m$ and $n$ represents the number of unique pairwise comparisons between stimuli, and unique pairwise comparison per individuals, which corresponds to the number of ways to choose two elements from $U_{A}$ and $U_{I}$, typically given by:

$$
m = {A \choose 2} = \frac{A!}{2!(A-2)!} ;\quad n = {I \choose 2} = \frac{I!}{2!(I-2)!}
$$


<!-- Because it quickly becomes intractable, and without loosing generality, the (latent) "perceived" and "true" discriminal processes for the stimuli can be depicted in a vector for each judge, as in   -->


::: {#fig-cj05 layout-nrow=2}

::: {#fig-cj05_scm fig-align="center" fig-pos="H"}
$$
\begin{aligned}
  O_{PRQ} & := f_{O}(D_{PRQ}) \\
  D_{PRQ} & := f_{D}(PT_{PRQ}) \\
  PT_{PRQ} & := f_{PT}(T_{AI}, B_{KJ})
\end{aligned}
$$
...
![DAG](/images/png/CJ_TM_05.png){#fig-cj05_dag fig-align="center" fig-pos="H" width=44%}

CJ's outcome, discriminal difference, discriminal processes and judges' biases
:::


<!-- ![](/images/png/CJ_TM_06.png){#fig-cj06_dag fig-align="center" fig-pos="H" width=59%} -->

<!-- ![](/images/png/CJ_TM_07.png){#fig-cj07_dag fig-align="center" fig-pos="H" width=68%} -->

<!-- ![](/images/png/CJ_TM_08.png){#fig-cj08_dag fig-align="center" fig-pos="H" width=68%} -->

<!-- ![](/images/png/CJ_TM_09.png){#fig-cj09_dag fig-align="center" fig-pos="H" width=68%} -->


::: {#fig-cj10 layout-nrow=2}

::: {#fig-cj10_scm fig-align="center" fig-pos="H"}
$$
\begin{aligned}
  O_{PRQ} & := f_{O}(D_{PRQ}) \\
  D_{PRQ} & := f_{D}(PT_{PRQ}) \\
  PT_{PRQ} & := f_{PT}(T_{AI}, B_{KJ}) \\
  T_{AI} & := f_{T}(T_{I}, X_{A}, e_{A}) \\
  T_{I} & := f_{T}(X_{I}, e_{I}) \\
  B_{KJ} & := f_{B}(B_{J}, Z_{K}, e_{K}) \\
  B_{J} & := f_{B}(Z_{J}, e_{J}) 
\end{aligned}
$$
...
![DAG](/images/png/CJ_TM_10.png){#fig-cj10_dag fig-align="center" fig-pos="H" width=68%}
...
<!-- ![](/images/png/CJ_TM_11.png){#fig-cj11_dag fig-align="center" fig-pos="H" width=80%} -->
...
$$
\begin{aligned}
  O^{S}_{PRQ} & := f_{O}(O_{PRQ}, S_{I}, S_{A}, S_{K}, S_{J}) \\
  O_{PRQ} & := f_{O}(D_{PRQ}) \\
  D_{PRQ} & := f_{D}(PT_{PRQ}) \\
  PT_{PRQ} & := f_{PT}(T_{AI}, B_{KJ}) \\
  T_{AI} & := f_{T}(T_{I}, X_{A}, e_{A}) \\
...
![DAG](/images/png/CJ_TM_12.png){#fig-cj12_dag fig-align="center" fig-pos="H" width=80%}
...
<!-- ![](/images/png/CJ_TM_13.png){#fig-cj13_dag fig-align="center" fig-pos="H" width=90%} -->
...
$$
\begin{aligned}
  O^{C}_{PRQ} & := f_{C}(O^{S}_{PRQ}, C) \\
  O^{S}_{PRQ} & := f_{S}(O_{PRQ}, S_{I}, S_{A}, S_{K}, S_{J}) \\
  O_{PRQ} & := f_{O}(D_{PRQ}) \\
  D_{PRQ} & := f_{D}(PT_{PRQ}) \\
  PT_{PRQ} & := f_{PT}(T_{AI}, B_{KJ}) \\
  T_{AI} & := f_{T}(T_{I}, X_{A}, e_{A}) \\
...
![DAG](/images/png/CJ_TM_14.png){#fig-cj14_dag fig-align="center" fig-pos="H" width=90%}
...
![DAG](/images/png/CJ_TM_15.png){#fig-cj15_dag fig-align="center" fig-pos="H" width=90%}
...
# Extending the BTL model {#sec-statistical}
...
<!-- ### The general idea {#sec-appendix-B1 .appendix appendix-style=plain} -->
...
*Counterfactuals* are hypothetical scenarios that are *contrary to fact*, where alternative outcomes resulting from a given cause are neither observed nor observable [@Neal_2020; @Counterfactual_2024]. The structural approach to causal inference [@Pearl_2009; @Pearl_et_al_2016] provides a formal framework for defining counterfactuals. For instance, in the scenario described in @sec-appendix-A, the approach begins by defining the *individual causal effect* (ICE) as the difference between each student's potential outcomes, as in @eq-ICE. 
...
### Diving into the specifics {#sec-appendix-B2 .appendix appendix-style=plain}
...
#### The five fundamental block for SCMs and DAGs {#sec-appendix-B21 .appendix appendix-style=plain}
...
#### The probabilistic implications of these blocks {#sec-appendix-B22 .appendix appendix-style=plain}
...
The five fundamental building blocks described in @sec-appendix-B21 clearly illustrate which (in)dependencies can SMCs and DAGs predict. For instance, applying the BNF to the causal structure shown in the SCM [-@fig-scm_bb1] and DAG [-@fig-mdag_bb1] enables researchers to express the joint probability distribution of the observed variables as $P(X_{1}, X_{3}) = P(X_{1}) P(X_{3})$, supporting the theoretical assumption that the observed variables $X_{1}$ and $X_{3}$ are unconditionally independent ($X_{1} \dsep X_{3}$) [@Neal_2020, pp. 24]. Conversely, when $X_{3}$ is unconditionally dependent on $X_{1}$ ($X_{1} \ndsep X_{3}$), as depicted in the SCM [-@fig-scm_bb2] and DAG [-@fig-mdag_bb2], the BNF express their joint probability distribution as $P(X_{1}, X_{3}) = P(X_{3} \cond X_{1}) P(X_{1})$. Notably, these descriptions demonstrate the clear correspondence between the structural equations illustrated in @sec-appendix-B21 and the CPDs.
...
#### From probability to causality {#sec-appendix-B23 .appendix appendix-style=plain}
...
Plausible causal structure the scenario outlined in @sec-appendix-B22.
:::


Researchers can then apply the *backdoor adjustment* to identify the ACE of $T$ on $Y$. They achieve this by first identifying the CACE of $T$ on $Y$ by conditioning on $X$, and then marginalizing this effect over $X$ to obtain the ACE. This process is expressed in @eq-mACE2 (see @sec-appendix-B). 
...
#### The estimation process {#sec-appendix-C .appendix appendix-style=plain}
...
Nevertheless, researchers can manage the complexity of the likelihood by assuming specific local (in)dependencies among variables. SCMs and DAGs provide a formal framework to represent these assumptions, as detailed in @sec-appendix-B22. These assumptions improve model tractability and simplify the estimation process by enabling the derivation of the BNF of the likelihood (@eq-like_BNF). With this simplified structure, any probabilistic programming language can model the system and compute the parameter's posterior distribution using @eq-bayes_rule.
