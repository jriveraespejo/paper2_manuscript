...
[SCM -@fig-cj03_scm] illustrates the preliminary integration of theoretical principles into the conceptual-population model. This SCM maps the relationship between the conceptual-population outcome, $(O^{cp}_{abhijk})$, and several related variables. The subscripts $a$ and $b$ represent the compared texts (stimuli), while the indices $h$ and $i$ label the students who authored them (individuals). Moreover, the index $j$ refers to the judge, and $k$ specifies the judgment number, accounting for experimental conditions where judges compare the same texts written by the same students multiple times, i.e., a *repeated measures design* [@Lawson_2015, pp. 366-376]. It is important to note that the indexing system allows for comparisons between different texts written by the same student ($h = i$, $a \neq b$) and between texts written by distinct students ($h \neq i$, where $a = b$ is possible). However, it excludes cases where judges compare a student’s text to itself ($h = i$, $a = b$), as such a comparison lacks practical meaning.

The other variables in [SCM -@fig-cj03_scm] include the texts' discriminal processes $(T_{ahjk}, T_{bijk})$ and their discriminal difference $(D_{abhijk})$, in line with Thurstone's theory (see @sec-thurstone_theory). Additionally, based on the arguments presented in @sec-theory-issue1b and the recommendations of @Andrich_1978 and @Wainer_et_al_1978, the SCM incorporates the judges' biases $(B_{kj})$. Together with the outcome, these variables form the preliminary set of endogenous variables, $V = \{ O_{abhijk}, D_{abhijk}, T_{ahjk}, T_{bijk}, B_{kj} \}$. Finally, the SCM shows the preliminary set of structural equations, $F = \{ f_{O}, f_{D} \}$, which define the non-parametric dependencies among these variables.
...
@fig-cj03_dag displays the DAG associated with [SCM -@fig-cj03_scm]. The graph shows the direction of causal influences among variables, reflecting the assumptions of Thurstone's theory and CJ's experiments. It also distinguishes between observable endogenous variables, such as the outcome, and variables considered latent according to the theory and practice of CJ, such as the text's discriminal processes, their discriminal difference, and the judges' biases.

<!-- 6. The problem of continuing this way -->
Although SCMs and DAGs do not necessitate specifying the data structure of the modeled system, defining these structures can sometimes enhance their clarity and simplify their notation. Thus, we organize certain indices into vectors or matrices. First, we define the matrices $IA$ and $JK$ as follows:
...
Here, the $RPUV$ matrix is also composed of natural numbers $\mathbb{N}$, and has a dimension of ${n_{I} \cdot n_{A} \choose 2} \cdot n_{J} \cdot n_{K}$ rows and $6$ columns. The dimension of the matrix results from the conceptual-population model's assumption that researchers have access to data representing all repeated comparisons made by every available judge for each pair of stimuli produced by each pair of individuals in the population. Here, the term ${n_{I} \cdot n_{a} \choose 2}$ represents the Binomial coefficient, which quantifies the number of individual-stimuli comparison pairs, and it is defined as:

$$
{n_{I} \cdot n_{A} \choose 2} = \frac{ (n_{I} \cdot n_{A})! }{ 2! (n_{I} \cdot n_{A} - 2)! }
$$

Using the motivating example outlined in @sec-theoretical, we can visualize the shape of these structures. For illustrative purposes, assume the conceptual population consists of $n_{I} = 5$ individuals, each with $n_{A} = 2$ available stimuli. Additionally, assume that each pair of stimuli is evaluated by $n_{J} = 3$ judges, with each judge making $n_{K} = 3$ comparative judgments. Given this setup, the matrices in ([-@eq-mat12]) and ([-@eq-mat3]) are represented as follows:
...
With these matrices we can express an equivalent version of [SCM -@fig-cj03_scm] and [DAG -@fig-cj03_dag] in a vectorized form.
