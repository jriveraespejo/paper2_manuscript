...
This section uses the structural approach to causal inference [@Pearl_2009; @Pearl_et_al_2016] to articulate a theoretical model that captures the core principles of Thurstone's theory. The model also incorporates various assessment design features relevant to CJ experiments, such as the selection of judges, stimuli, and comparisons. Finally, the section employs Bayesian statistical methods to transform these theoretical and practical elements into a statistical model that facilitates the analysis of pairwise comparison data. See @sec-appendix for an overview of the statistical and causal inference concepts required for the development of this section.
...
<!-- 1. The general probabilistic description of data -->

<!-- Maximum a posteriori (MAP) estimates are the set of parameters that maximizes the probability of $\theta$ given the data. By Bayes theorem the MAP estimate can be obtained by maximizing the following conditional distribution: -->

<!-- $$ -->
<!-- P(\theta| X_{1}, X_{2}, \dots, X_{n}) = \frac{P(X_{1}, X_{2}, \dots, X_{n} | \theta) \cdot P(\theta)}{P(X_{1}, X_{2}, \dots, X_{n})} -->
<!-- $$ -->
<!-- where $P(\theta)$ is the prior distribution for the parameters $\theta$ and $P(X_{1}, X_{2}, \dots, X_{n})$ is the probability of the data averaged over all parameters. Since the denominator is independent of $\theta$, the Bayesian estimator is obtained by maximizing $P(X_{1}, X_{2}, \dots, X_{n} | \theta)$ with respect to $\theta$. This indicates that when researchers model association, they generally model the joint probability of the data given the parameters $\theta$. -->

<!-- This join probability can be further re-expressed using the chain rule of probability as $P(X_{1}, X_{2}, \dots, X_{n}|\theta) = P(X_{1}|\theta) \prod^{n}_{i=2} P(X_{i} | X_{i-1}, \dots, X_{1}, \theta )$. However, from this description, it is easy to see how quickly the parametrization becomes intractable as the number of observed variables $n$ grows [@Neal_2020]. To deal with this complexity, researchers typically simplify the modeling process by assuming local (in)dependencies between variables.  -->



<!-- 2. But what does the building blocks mean? -->
<!-- The five building blocks help to express joint distributions very efficiently, by formalizing local (in)dependencies into (probabilistic) statistical models [@Neal_2020]. Notably, this formalization rest on two key assumptions: the local Markov and causal edges assumptions. The local Markov assumption states that given its parents in the DAG, a node is independent of all its non-descendants. This lead to the Bayesian Network factorization. However, this assumption only give us information about independencias in the distribution that the DAG implies, we need also to know about the dependencies. The causal edges assumption states that in a DAG, every parent is a direct cause of all its children. In this sense this assumption is indicating that parents and their children are dependent, and more precisely, they are causally dependent [@Neal_2020].  -->
