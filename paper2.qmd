...
Nevertheless, despite the increasing number of studies on CJ, research in this domain remains unsystematic and fragmented, leaving several critical issues unresolved. This study addresses two prominent issues in CJ literature that can undermine the reliability and validity of CJ's trait estimates. The first problem concerns the overreliance on Thurstone's Case V assumptions in the statistical analysis of CJ data. The second involves the apparent disconnect between CJ's approach to trait measurement and hypothesis testing.

<!-- 4. what is the organization of the study? -->
The study is organized into six main sections. An overview of Thurstone's theory is presented in @sec-thurstone_theory, followed by a detailed examination of the identified issues in @sec-theory-issues. In @sec-theoretical, the study proposes a theoretical model for CJ that addresses these challenges. The model incorporates Thurstone's core principles alongside key assessment design features relevant to CJ experiments, such as judges, stimuli, and comparison selections. In @sec-statistical, these practical and theoretical considerations are translated into a statistical model for the analysis of pairwise comparison data. Finally, @sec-discussion explores the implications of the study's findings and the avenues for future exploration, and @sec-conclusion provides a summary of the key insights.
...
This section discusses the two prominent issues in CJ literature that can undermine the reliability and validity of CJ's trait estimates. @sec-theory-issue1 examines the overreliance on Thurstone's Case V assumptions in the statistical analysis of CJ data. @sec-theory-issue2 focuses on the apparent disconnect between CJ's approach to trait measurement and hypothesis testing.
...
In aggregate, researchers' overreliance on Thurstone's Case V assumptions in the statistical analysis of CJ data can compromise the reliability of CJ's trait estimates. This overreliance, coupled with the apparent disconnect between CJ's approach to trait measurement and hypothesis testing, may also undermine the validity of these estimates [@Perron_et_al_2015, pp. 2]. Consequently, addressing these issues and examining the factors influencing CJ experiments by adopting a more systematic and integrated approach could offer several statistical and measurement benefits.
...
# Updating the theoretical model {#sec-theoretical}

<!-- This study examines two such problems that can compromise the reliability and validity of CJ's trait estimates. -->

<!-- This section aims to refine CJ's theoretical model using the *structural approach* to causal inference with the purpose of addressing the two.  -->

<!-- This approach, as outlined in @sec-theory-theoretical, defines a model that integrates the core principles of Thurstone's theory, as well as other key assessment design features relevant to CJ experiments, such as the selection of judges, stimuli, and the number of comparisons.  -->
...
# From theory to statistics {#sec-statistical}
...
Identification is essential because it is a necessary condition for *consistency*. An estimator is *consistent* if it converges to the "true" value of an estimand as the sample size approaches infinity [@Everitt_et_al_2010]. Without identification, an estimator cannot achieve consistency, rendering it incapable of reliably computing the estimand, even with "infinite" and error-free data. Consequently, deriving meaningful insights about the estimand from finite data becomes impossible [@Schuessler_et_al_2023, pp. 5]. Therefore, to ensure accurate and reliable estimates, researchers prioritize estimators with desirable identification properties. For instance, the Z-test is a widely used estimator for comparing group proportions, yielding accurate estimates when its underlying assumptions are satisfied [@Kanji_2006]. Furthermore, researchers can interpret estimates from the Z-test as causal, provided the data is collected through a randomized experiment.
