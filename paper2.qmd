...
@fig-cj12 illustrates the sample-comparison model, incorporating the sample mechanism. Following the convention of @McElreath_2020 [pp. 499-516], the [DAG -@fig-cj12_dag] shows several conditioned variables $(S_{R}$, $S_{P}$, $S_{U}$, and $S_{V})$ that causally influence the sample-comparison outcome $O^{sc}_{RPUV}$, reflecting how the outcome is a sample drawn from the conceptual population. The DAG also depicts the conceptual-population outcome $O^{cp}_{RPUV}$ is now an unobserved variable, emphasizing the researchers' inability to access such an outcome.
