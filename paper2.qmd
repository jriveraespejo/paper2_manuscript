...
According to the theory, discrepancies in the discriminal dispersions of stimuli shape the distribution of the discriminal difference, exerting a direct influence on the outcome of pairwise comparisons. A thought experiment can illustrate this concept. In this experiment, the researcher observes the discriminal processes for the texts depicted in @fig-discriminal_process. Furthermore, the experiment assumes that the discriminal dispersion for Text A remains constant and that the texts are uncorrelated $(\rho=0)$. In this scenario, @fig-dispersion reveals that an increase in the uncertainty associated with the perception of Text B in comparison to Text A, $(\sigma_{B}-\sigma_{A})$, broadens the distribution of their discriminal difference. This broadening affects the probability area where the discriminal difference distinctly favors Text B over Text A, expressed as $P(B > A)$, ultimately influencing the comparison outcome. Additionally, the figure reveals that when the discriminal dispersions of the texts are equal $(\sigma_{B}-\sigma_{A}=0)$, the discriminal difference is more narrow compared to situations where their dispersions differ. Consequently, the discriminal difference is more likely to favor Text B over Text A (shaded gray area)
...
In aggregate, the overreliance on Thurstone's Case V assumptions in the statistical analysis of CJ data can compromise the reliability of CJ's trait estimates. This overreliance, coupled with the apparent disconnect between CJ's approach to trait measurement and hypothesis testing, may also undermine the validity of these estimates [@Perron_et_al_2015, pp. 2]. Consequently, addressing these issues by adopting a more systematic and integrated approach could offer several statistical and measurement benefits.
...
For instance, researchers might aim to answer the question: "To what extent do different teaching methods influence students' ability to produce high-quality written texts?" To investigate this, they could design a CJ experiment by randomly assigning students to two groups, each receiving a different teaching method. Judges would then compare pairs of students' written texts, producing a dichotomous outcome that reflects the relative quality of each text. In this scenario, researchers can reformulate the research question as the estimand: "*On average*, is there a difference in the ability to produce high-quality written texts between the two groups of students?". Researchers would then use estimates from the BTL model, or its transformations, to approximate this estimand. 

<!-- 4. But why identification is so important? -->
However, @sec-theory-issues presents strong evidence that the BTL model has several statistical and measurement limitations. These limitations hinder the model's ability to identify distinct estimands relevant to CJ inquiries, including the one presented in the previous paragraph. Identification is crucial because it is a necessary condition for ensuring consistent estimators. Without identification, researchers cannot achieve consistency even with "infinite" and error-free data. In other words, the estimates derived from the estimator fail to converge to the "true" value of the estimand as the data size approaches infinity [@Everitt_et_al_2010]. As a result, deriving meaningful insights from finite data also becomes unattainable [@Schuessler_et_al_2023, pp. 5].
...
As a result, this section addresses the identified limitations by extending Thurstone's model through a systematic and integrated approach based on the structural approach to causal inference. Specifically, it leverages the capabilities of this approach to formalize the combination of the core theoretical principles outlined in @sec-thurstone_theory with key assessment design features relevant to CJ experiments. These features include the selection of judges, stimuli, and comparisons. Combining these components clarifies the interactions among all actors and processes involved in CJ experiments. Accordingly, @sec-theory-theoretical_P incorporates the theoretical principles into a *conceptual-population model*. Meanwhile, @sec-theory-theoretical_SC integrates the assessment design features into a *sample-comparison model*.
...
<!-- 1. The SCMs -->
Before integrating the theoretical principles from @sec-thurstone_theory, it is important to define SCMs and DAGs. SCMs are formal mathematical models defined by a set of *endogenous* variables, a set of *exogenous* variables, and a set of functions [@Pearl_2009; @Cinelli_et_al_2020]. Endogenous variables are those whose causal mechanisms a researcher chooses to model [@Neal_2020]. In contrast, exogenous variables represent *errors* or *disturbances* arising from omitted factors that the investigator chooses not to model explicitly [@Pearl_2009, pp. 27,68]. Lastly, the functions, referred to as *structural equations*, express the endogenous variables as non-parametric functions of other variables. These functions use the symbol '$:=$' to denote the asymmetrical causal dependence between variables and the symbol '$\dsep$' to represent *d-separation*, a concept akin to (conditional) independence.

For instance, following the scenario outline in @sec-theoretical, @fig-cj04_scm displays the SCM that relates the outcome of the CJ experiment $(O_{abhijk})$, to several endogenous variables $V$. These variables include the discriminal difference $(D_{abhijk})$, perceived discriminal processes $(PT_{ahjk}, PT_{bijk})$, the "true" discriminal processes $(T_{ahjk}, T_{bijk})$, and the judges' biases $(B_{kj})$, formally expressed as $V = \{D_{abhijk}, PT_{ahjk}, PT_{bijk}, T_{ahjk}, T_{bijk}, B_{kj}\}$. The structural equations $\{f_{O},f_{D},f_{PT}\}$ define the dependencies among these variables. Notably, in this scenario, the judges produce a dichotomous outcome $(O_{abhijk}) = \{0,1\}$. Moreover, the indices $\{a,b\}$ represent the compared texts, $\{h,i\}$ label the students who wrote these texts, $j$ denotes the judge, and $k$ represents the comparison number. Regarding the $k$ index, the index accounts for cases in which judges perform multiple comparisons of the same texts written by the same students, as the one discussed in @sec-theoretical.

<!-- 2. The DAGs -->
Notably, every SCM has an associated DAG [@Pearl_et_al_2016; @Cinelli_et_al_2020]. A DAG is a graph consisting of nodes connected by edges, where the nodes represent random variables. The term *directed* means that the edges extend from one node to another, with arrows indicating the direction of causal influence. The term *acyclic* implies that the causal influences do not form loops, ensuring the influences do not cycle back on themselves [@McElreath_2020]. DAGs represent observed variables as solid black circles, while they use open circles for unobserved (latent) variables [@Morgan_et_al_2014]. While the *standard representation* of DAGs typically omits exogenous variables for simplicity, the *magnified representation* includes them, offering offering the advantage of highlighting potential issues related to conditioning and confounding [@Cinelli_et_al_2020]. 
