...
# Developing Thurstone's theory {#sec-theoretical}
...
In line with Thurstone's theory, the other variables present in [SCM -@fig-cj03_scm] include the texts' discriminal processes $(T_{ha}, T_{ib})$ and their discriminal difference $(D_{hiabjk})$ (see @sec-thurstone_theory). Additionally, based on the arguments presented in @sec-theory-issue1b and the recommendations of @Andrich_1978 and @Wainer_et_al_1978, the [SCM -@fig-cj03_scm] incorporates the judges' biases $(B_{kj})$. Together with the outcome, these variables form the preliminary set of endogenous variables, $V = \{ O_{hiabjk}, D_{hiabjk}, T_{ha}, T_{ib}, B_{kj} \}$. Finally, the SCM shows the preliminary set of structural equations, $F = \{ f_{O}, f_{D} \}$, which define the non-parametric dependencies among these variables.
...
<!-- 4. The DAGs -->
Notably, every SCM has an associated DAG [@Pearl_et_al_2016; @Cinelli_et_al_2020]. A DAG is a graph consisting of nodes connected by edges, where nodes represent random variables. The term *directed* indicates that edges or arrows extend from one node to another, showing the direction of causal influence. The term *acyclic* means that the causal influences do not form loops, ensuring the influences do not cycle back on themselves [@McElreath_2020]. DAGs conventionally depict observed variables as solid black circles and unobserved (latent) variables as open circles [@Morgan_et_al_2014]. Moreover, they often omit exogenous variables for simplicity, following the *standard representation*. However, this section adopts the *magnified representation* of DAGS, as including exogenous variables can enhance clarity and reveal potential issues related to conditioning and confounding [@Cinelli_et_al_2020].

<!-- 5. the preliminary DAG for CJ -->
@fig-cj03_dag displays the DAG associated with [SCM -@fig-cj03_scm]. This graph illustrates the direction of causal influences among variables, reflecting the assumptions of Thurstone's theory and the practical aspects of CJ experiments. It also distinguishes between observable endogenous variables, such as the outcome, and variables considered latent within the CJ framework, such as the text's discriminal processes, their discriminal difference, and the judges' biases.

<!-- 6. Why is useful to declare the data structure? -->
Although specifying a system's data structure is not mandatory for using SCMs and DAGs, defining one can sometimes enhance clarity and facilitate the description of the system. Thus, to simplify the description of the CJ system, we first define the vectors $I$ and $J$, along with the matrices $IA$ and $JK$, as follows:
...
Readers can better visualize the structure of these vectors and matrices by considering small values for the parameters corresponding to number of individuals $(n_{I} = 5)$, the number of available stimuli per individual $(n_{A} = 2)$, the total number of judges $(n_{J} = 3)$, and the number of judgments each judge performs $(n_{K} = 3)$. Using these values, the matrices described in equations ([-@eq-mat12]) and ([-@eq-mat3]) take the following form:
...
Thus, using the data structure defined by the matrices in equations ([-@eq-mat12]) and ([-@eq-mat3]), we can now re-express [SCM -@fig-cj03_scm] and [DAG -@fig-cj03_dag] in an equivalent vectorized form, as shown in @fig-cj04. In this representation, the outcome $O^{cp}_{RPUV}$, the texts' discriminal difference $D_{RPUV}$, their discriminal processes $T_{IA}$, and the judges' biases $B_{JK}$ are no longer scalar values but vectors. These vectors capture all the values from the conceptual population. Specifically, $O^{cp}_{RPUV}$ and $D_{RPUV}$ are vectors of length ${n_{I} \cdot n_{A} \choose 2} \cdot n_{J} \cdot n_{K}$, while $T_{IA}$ and $B_{JK}$ are vectors of length $(n_{I} \cdot n_{A})$ and $(n_{J} \cdot n_{K})$, respectively.
...
<!-- 9. what else? -->
Building on the arguments developed in @sec-theory-issue2 and the principles of Structural Equation Modeling (SEM) [@Hoyle_et_al_2023, pp. 138] and Item Response Theory (IRT) [@Fox_2010, chap. 6; @vanderLinden_et_al_2017_I, chap. 24], the conceptual-population model includes two hierarchical structural components. Each component defines how different observed or latent variables influence the primary latent variable of interest [@Everitt_et_al_2010]. This addition facilitates analyses that integrate the trait measurement and hypothesis testing processes, simultaneously accounting for the traits and their associated uncertainties, thereby reducing bias and enhancing the measurement precision. 
