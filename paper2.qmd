...
The five fundamental building blocks described in @sec-appendix-C clearly illustrate which (in)dependencies can SMCs and DAGs predict. For instance, applying the BNF to the causal structure shown in the SCM [-@fig-scm_bb1] and DAG [-@fig-mdag_bb1] supports the theoretical assumption that the observed variables $X_{1}$ and $X_{3}$ are unconditionally independent or d-separated ($X_{1} \dsep X_{3}$). This conclusion follows from expressing their joint probability distribution as $P(X_{1}, X_{3}) = P(X_{1}) P(X_{3})$ [@Neal_2020, pp. 24]. Conversely, when $X_{3}$ is unconditionally dependent on $X_{1}$ ($X_{1} \ndsep X_{3}$), as depicted in the SCM [-@fig-scm_bb2] and DAG [-@fig-mdag_bb2], the BNF express their joint probability distribution as $P(X_{1}, X_{3}) = P(X_{3} \cond X_{1}) P(X_{1})$. Notably, these descriptions demonstrate the clear correspondence between the structural equations illustrated in @sec-appendix-C and the CPDs.

Beyond the insights gained from two-node structures, researchers can uncover more nuanced patterns of (in)dependencies from chains, forks, and colliders. These (in)dependencies apply to any data generated by these structures. For instance, applying the BNF to the chain structure depicted in the SCM [-@fig-scm_bb3] and DAG [-@fig-mdag_bb3] allow researchers to represent the joint distribution for the observed variables as $P(X_{1},X_{2},X_{3}) =$ $P(X_{1}) P(X_{2} \cond X_{1}) P(X_{3} \cond X_{2})$. This expression implies that $X_{1}$ and $X_{3}$ are unconditionally dependent $(X_{1} \ndsep X_{3})$, but conditionally independent when controlling for $X_{2}$ $(X_{1} \dsep X_{3} \cond X_{2})$. Moreover, in the fork structure shown in the SCM [-@fig-scm_bb4] and DAG [-@fig-mdag_bb4], researchers can express the joint distribution of the observed variables as $P(X_{1},X_{2},X_{3}) =$ $P(X_{1} \cond X_{2}) P(X_{2}) P(X_{3} \cond X_{2})$. This expression allows researchers to further infer that $X_{1}$ and $X_{3}$ are unconditionally dependent $(X_{1} \ndsep X_{3})$, but conditionally independent when controlling for $X_{2}$ $(X_{1} \dsep X_{3} \cond X_{2})$. Finally, researchers analyzing the collider structure illustrated in the SCM [-@fig-scm_bb5] and DAG [-@fig-mdag_bb5] can express the joint distribution of the observed variables as $P(X_{1},X_{2},X_{3}) =$ $P(X_{1}) P(X_{2} \cond X_{1}, X_{3}) P(X_{3})$. This representation allows researchers to infer that $X_{1}$ and $X_{3}$ are unconditionally independent $(X_{1} \dsep X_{3})$, but conditionally dependent when controlling for $X_{2}$ $(X_{1} \ndsep X_{3} \cond X_{2})$. The authors @Pearl_et_al_2016 [pp. 37, 40, 41] and @Neal_2020 [pp. 25–26] provide the mathematical proofs for these conclusions.

<!-- 5. Continuing with the motivating example -->
Researchers can revisit the scenario in @sec-appendix-B using these insights. By applying the BNF to the SCM [-@fig-scm_example1] and DAG [-@fig-mdag_example1a] structures, they can represent the joint probability of the observed variables as $P(Y, T, X) = P(Y \cond T, X) P(T \cond X) P(X)$. Assuming the *direct effect* of $T$ on $Y$ ($T \rightarrow Y$, black path) is absent, researchers can infer from the joint probability that $Y$ is not unconditionally independent of $T$ $(Y \dsep T)$. This dependency arises because a non-causal path exists between $T$ and $Y$ through $X$ ($T \leftarrow X \rightarrow Y$, gray path), indicating that $X$ is a confounder. However, researchers can observe that conditioning on $X$, depicted as a square in DAG [-@fig-mdag_example1a], results in $Y \dsep T \cond X$, eliminating the confounding effect and allowing the identification of the direct effect $T \rightarrow Y$ if it exists.
...
![Conditioned DAG](images/png/mdag_example1a.png){#fig-mdag_example1a fig-align="center" width=65%}
...
In addition to the assumptions required to encode probabilistic information, SCMs and DAGs rely on the modularity assumption to represent interventions $P(Y_{i} \cond do(T_{i}=t))$ (see @sec-appendix-B). Defining these interventions enables researchers to identify the ACE from associational quantities through statistical conditioning, effectively disentangling causation from mere association.

The modularity assumption underpins the concepts of Truncated Factorization and manipulated graphs, which are essential for representing interventions within SCMs and DAGs. The *modularity assumption* posits that intervening on a node modifies only the causal mechanism of that node, leaving others unchanged [@Neal_2020, pp. 34]. Building on this principle, the *Truncated Factorization* (TF) simulates physical interventions by removing specific functions from the conceptual model and substituting them with constants, preserving the rest of the structure unchanged [Pearl_2010]. The probabilistic implications of this factorization are formalized in @eq-trunc_factor, where $S$ denotes the subset of variables $X_{p}$ directly influenced by the intervention. Complementing the TF, *manipulated graphs* are DAGs where specific edges have been removed to reflect the effects of interventions [@Neal_2020, pp. 34].
...
Using the TF, researchers can define the *backdoor adjustment*, a method for identifying causal effects. This adjustment specifies that if a variable $X_{p} \in S$ serves as a *sufficient adjustment set* for the effect of $X_{a}$ on $X_{b}$, then the ACE can be identified using @eq-backdoor [@Pearl_2009; @Pearl_et_al_2016; @Morgan_et_al_2014] (see @sec-appendix-B).
...
![Manipulated DAG](images/png/mdag_example1b.png){#fig-mdag_example1b fig-align="center" width=65%}
