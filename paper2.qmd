...
For instance, a motivating example that illustrates these concepts throughout the document could involve researchers aiming to answer the question: "To what extent do different teaching methods influence students' ability to produce high-quality written texts?" To investigate this, they could design a CJ experiment by randomly assigning students to two groups, each receiving a different teaching method. Judges would then compare pairs of students' written texts, producing a dichotomous outcome that reflects the relative quality of each text. Based on this setup, researchers can reformulate the research question as the estimand: "*On average*, is there a difference in the ability to produce high-quality written texts between the two groups of students?". Following current CJ practices, researchers would then rely on estimates from the BTL model, or its transformations, to approximate this estimand.

<!-- 4. But why identification is so important? -->
However, @sec-theory-issues presents compelling evidence that the BTL model has several statistical and measurement limitations. These limitations hinder the model's ability to identify distinct estimands relevant to CJ inquiries, including the one from the motivating example. Identification is crucial because it is a necessary condition for ensuring consistent estimators. Without identification, researchers cannot achieve *consistency* even with "infinite" and error-free data. In other words, the estimates derived from the estimator would fail to converge to the "true" value of the estimand as the data size approaches infinity [@Everitt_et_al_2010]. As a result, deriving meaningful insights from finite data also becomes unattainable [@Schuessler_et_al_2023, pp. 5].
...
As a result, this section addresses the identified limitations by extending Thurstone's general form (see @tbl-thurstone_cases) through a systematic and integrated approach based on the structural approach to causal inference. Specifically, it leverages the capabilities of this approach to formalize the combination of the core theoretical principles outlined in @sec-thurstone_theory with key assessment design features relevant to CJ experiments, such as the selection of judges, stimuli, and comparisons. The extended model clarifies the interactions among all actors and processes involved in CJ experiments. Furthermore, it allows researchers to shift the current comparative data analysis paradigm from accepting the model's assumptions as given to testing these assumptions for specific comparative data. For details on these assumptions, refer to @sec-theory-issues. Accordingly, @sec-theory-theoretical_P incorporates the theoretical principles into what the study calls the *conceptual-population model*. Meanwhile, @sec-theory-theoretical_SC integrates the assessment design features into what the study terms the *sample-comparison model*.
...
<!-- 1. The SCMs -->
Before integrating the theoretical principles from @sec-thurstone_theory, the study defines SCMs and DAGs. SCMs are formal mathematical models characterized by a set of *endogenous* variables $V$, a set of *exogenous* variables $E$, and a set of functions $F$ [@Pearl_2009; @Cinelli_et_al_2020]. Endogenous variables are those whose causal mechanisms a researcher chooses to model [@Neal_2020]. In contrast, exogenous variables represent *errors* or *disturbances* arising from omitted factors that the investigator chooses not to model explicitly [@Pearl_2009, pp. 27,68]. Lastly, the functions, referred to as *structural equations*, express the endogenous variables as non-parametric functions of other variables. These functions use the symbol '$:=$' to denote the asymmetrical causal dependence between variables and the symbol '$\dsep$' to represent *d-separation*, a concept akin to (conditional) independence.

<!-- 2. the preliminary SCM for CJ -->
@fig-cj04 illustrates the preliminary integration of the theoretical principles underlying the motivating example. Specifically, [SCM -@fig-cj04_scm] displays the relationship between the outcome of the CJ experiment, $O_{abhijk}$, to several other variables. Here, the indices $\{a,b\}$ represent the compared texts, $\{h,i\}$ label the students who wrote these texts, $j$ denotes the judge, and $k$ accounts for cases where the judges perform multiple comparisons of the same texts written by the same students, i.e., the comparison number for a *repeated measures design* [@Lawson_2015, pp. 366-376]. The other variables related to the outcome include the discriminal difference $(D_{abhijk})$, the perceived discriminal processes $(PT_{ahjk}, PT_{bijk})$, the "true" discriminal processes $(T_{ahjk}, T_{bijk})$, and the judges' biases $(B_{kj})$. Together with the outcome, these variables form the preliminary set of endogenous variables $V = \{O_{abhijk}, D_{abhijk}, PT_{ahjk}, PT_{bijk}, T_{ahjk}, T_{bijk}, B_{kj}\}$. Finally, the preliminary set of structural equations $F = \{f_{O},f_{D},f_{PT}\}$ defines the dependencies among these variables. 
...
[DAG -@fig-cj04_dag] displays the preliminary DAG associated with [SCM -@fig-cj04_scm]. The graph shows the direction of causal influences among variables, reflecting the assumptions of Thurstone's theory. It also distinguishes between observable endogenous variables, such as the outcome $O_{abhijk}$, and variables considered latent according to the theory and practice of CJ, such as the discriminal difference $(D_{abhijk})$, the perceived discriminal processes $(PT_{ahjk}, PT_{bijk})$, the "true" discriminal processes $(T_{ahjk}, T_{bijk})$, and the judges' biases $(B_{kj})$.

<!-- Notice,  -->
<!-- the perceived discriminal processes $(PT_{ahjk}, PT_{bijk})$, the "true" discriminal processes $(T_{ahjk}, T_{bijk})$, and the judges' biases $(B_{kj})$ -->
...
<!-- To declare a matrix \( P \) in \( \mathbb{N}^2 \) that represents the paired combinations of elements in \( I \), follow these steps: -->

<!-- ### Definitions: -->
<!-- 1. \( I \) is a subset of the natural numbers, \( I \subseteq \mathbb{N} \). -->
<!-- 2. The paired combination of elements in \( I \) can be written as \( (i, j) \) for \( i, j \in I \). -->

<!-- ### Constructing the Matrix \( P \): -->
<!-- 1. Each row of \( P \) represents one pairing \( (i, j) \). -->
<!-- 2. If \( I = \{a_1, a_2, \ldots, a_n\} \), then \( P \) contains \( n^2 \) rows for all pairs \( (i, j) \), including repetitions where \( i = j \). -->
<!-- 3. Specifically, the matrix \( P \) is structured as: -->
<!--    \[ -->
<!--    P = \begin{bmatrix} -->
<!--    i_1 & j_1 \\ -->
<!--    i_2 & j_2 \\ -->
...
<!--    \end{bmatrix}, -->
<!--    \] -->
<!--    where \( i_k, j_k \in I \) for all \( k \), and each pair \( (i_k, j_k) \) corresponds to a combination of elements in \( I \). -->

<!-- ### Algorithm to Generate \( P \): -->
<!-- Given \( I = \{a_1, a_2, \ldots, a_n\} \): -->
<!-- 1. Loop over all \( i \in I \) (outer loop). -->
<!-- 2. For each \( i \), loop over all \( j \in I \) (inner loop). -->
<!-- 3. Append the pair \( (i, j) \) as a new row in \( P \). -->

<!-- ### Example: -->
<!-- If \( I = \{1, 2, 3\} \), then the matrix \( P \) is: -->
<!-- \[ -->
<!-- P = \begin{bmatrix} -->
<!-- 1 & 1 \\ -->
<!-- 1 & 2 \\ -->
...
<!-- 1. The matrix \( P \) can be defined as: -->
<!--    \[ -->
<!--    P = \{(i, j) \mid i \in I, j \in I\}. -->
<!--    \] -->
<!-- 2. In terms of dimensions, \( P \) has \( |I|^2 \) rows and 2 columns. -->

<!-- This formalization ensures that \( P \) represents all paired combinations of elements from \( I \) in \( \mathbb{N}^2 \). -->
