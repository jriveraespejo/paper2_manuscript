...
\begin{tikzpicture}
    ...
    \node[gray] at (-0.8,0) {$D_{R}$}; %[gray]
    %\draw[{Circle[open]}-{latex}{Circle},gray](-1,-0.3) to (1.1,-0.3); %,gray
    
    % complete outcome
    \node[gray] at (1.2,0) {$O^{cp}_{R}$}; %[gray]

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    % "true" discriminal process of unit
    \node[gray] at (-5.9,3.1) {$T_{I}$}; %[gray]
    \draw[{Circle[open]}-{latex},gray](-6,2.8) to (-4,1.64); %,gray
    
    % sub-units' cluster
    \node[font=\scriptsize,gray] at (-6,2.39) {Stimuli}; %,gray
    \draw[dashed,gray] (-6.5,0.4)--(-3.4,0.4)--(-3.4,2.19)--(-6.5,2.19)--(-6.5,0.39); %,gray
    
    % sub-units error
    \node[gray] at (-6,1.89) {$e_{IA}$}; %[gray]
    \draw[{Circle[open]}-{latex},gray](-6,1.59) to (-4,1.59); %,gray
    
    % predictors for sub-unit
    \node[gray] at (-6,0.93) {$X_{IA}$}; %[gray]
    \draw[{Circle}-{latex},gray](-6,0.59) to (-4,1.54); %,gray

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    % units' cluster
    \node[font=\scriptsize,gray] at (-7.4,4.5) {Individuals}; %,gray
    \draw[dashed,gray] (-7.9,0.3)--(-3.3,0.3)--(-3.3,4.3)--(-7.9,4.3)--(-7.9,0.3); %,gray
    
    % predictors for units
    \node[gray] at (-7.5,4) {$X_{I}$}; %[gray]
    \draw[{Circle}-{latex},gray](-7.5,3.7) to (-6,2.8); %,gray

    % units trait error
    \node[gray] at (-7.5,3.05) {$e_{I}$}; %[gray]
    \draw[{Circle[open]}-{latex},gray](-7.5,2.75) to (-6,2.75); %,gray

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    % judges' bias
    \node[gray] at (-5.9,-3.2) {$B_{J}$}; %[gray]
    \draw[{Circle[open]}-{latex},gray](-6,-2.9) to (-4,-1.95); %,gray

    % judgments' cluster
    \node[font=\scriptsize,gray] at (-6,-0.5) {Judgments}; %,gray
    \draw[dashed,gray] (-6.5,-2.5)--(-3.4,-2.5)--(-3.4,-0.7)--(-6.5,-0.7)--(-6.5,-2.5); %,gray
    
    % judgments' bias error 
    \node[gray] at (-6,-2.2) {$e_{JK}$}; %[gray]
    \draw[{Circle[open]}-{latex},gray](-6,-1.9) to (-4,-1.9); %,gray
    
    % predictors for judgments
    \node[gray] at (-6,-1.2) {$Z_{JK}$}; %[gray]
    \draw[{Circle}-{latex},gray](-6,-0.9) to (-4,-1.85); %,gray
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    % judges' cluster
    \node[font=\scriptsize,gray] at (-7.5,-0.1) {Judges}; %,gray
    \draw[dashed,gray] (-7.9,-4.3)--(-3.3,-4.3)--(-3.3,-0.3)--(-7.9,-0.3)--(-7.9,-4.3); %,gray
    
    % predictors for judges' bias
    \node[gray] at (-7.5,-4) {$Z_{J}$}; %[gray]
    \draw[{Circle}-{latex},gray](-7.5,-3.7) to (-6,-2.95); %,gray
    
    % judges' bias error
    \node[gray] at (-7.5,-3.2) {$e_{J}$}; %[gray]
    ...
    %\draw[{Circle}-{latex},gray](1,2.7) to (2.9,-0.2); %,gray
    
    % subunits sampling mechanism
    %\node[rectangle,draw] at (1,1) {$S_{P}$}; %,gray
    %\draw[{Circle}-{latex},gray](1,0.59) to (2.9,-0.25); %,gray
    
    % judgments sampling mechanism
    %\node[rectangle,draw] at (1,-1.3) {$S_{V}$}; %,gray
    %\draw[{Circle}-{latex},gray](1,-0.9) to (2.9,-0.35); %,gray
    
    % judges sampling mechanism
    %\node[rectangle,draw] at (1,-3.2) {$S_{U}$}; %,gray
    %\draw[{Circle}-{latex},gray](1,-2.8) to (2.9,-0.4); %,gray
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    % all sampling mechanism
    \node[rectangle,draw,gray] at (1,3.1) {$S$}; %,gray
    \draw[{Circle}-{latex},gray](1,2.7) to (2.9,-0.2); %,gray
    ...
    \draw[{Circle[open]}-{latex},gray](1,-0.3) to (2.9,-0.3); %,gray
